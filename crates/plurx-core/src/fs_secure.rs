...
/// uses a sealable memfd. macOS creates and unlinks a POSIX shared-memory
/// object, with an unlinked system-temporary file as the sandbox-compatible
/// fallback. Neither consumes free space from the media cache filesystem.
pub fn anonymous_memory_file() -> io::Result<tokio::fs::File> {
    ...
    let raw_fd = {
        ...
        let name = CString::new(format!("/plurx-{}", &identifier[..24]))
            .expect("generated shared-memory name has no NUL");
        // SAFETY: `name` is a valid, NUL-terminated shared-memory name and the
        // creation mode is present for O_CREAT. Darwin sets FD_CLOEXEC on the
        // returned descriptor itself.
        ...
        if shared_fd >= 0 {
            // SAFETY: `name` still identifies the object opened above.
            if unsafe { libc::shm_unlink(name.as_ptr()) } == 0 {
                shared_fd
            } else {
                let error = io::Error::last_os_error();
                // SAFETY: `shared_fd` is owned by this function on success.
                unsafe { libc::close(shared_fd) };
                return Err(error);
            }
        } else {
            let shared_error = io::Error::last_os_error();
            if shared_error.kind() != io::ErrorKind::PermissionDenied {
                return Err(shared_error);
            }

            // Sandboxed macOS processes may be denied named POSIX shared
            // memory. Keep the same pathname-free lifetime contract by
            // creating and immediately unlinking a private system-temp file.
            ...
            // SAFETY: the mutable buffer is NUL terminated, ends in six Xs,
            // and remains alive while mkstemp replaces those bytes in place.
            ...
            // SAFETY: `temporary_fd` is owned here and `temporary_name` names
            // the file mkstemp created. Either failure path closes the fd.
