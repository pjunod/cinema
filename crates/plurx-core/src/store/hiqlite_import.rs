...
const TABLES: &[TablePlan] = &[
    ...
    TablePlan {
        name: "library_channel_generations",
        columns: &[
            "id",
            "channel_id",
            "definition_revision",
    ...
    TablePlan {
        ...
        order_by: "generation_id, ordinal",
        minimum_schema: 55,
        import_filter: None,
        sealed_columns: &[],
        parent_first: false,
    ...
    TablePlan {
        name: "library_channel_favourites",
        columns: &["user_id", "channel_id", "created_at_ms"],
        order_by: "user_id, channel_id",
        minimum_schema: 55,
        import_filter: None,
        sealed_columns: &[],
        parent_first: false,
    ...
    TablePlan {
        name: "library_channel_subject_decisions",
        columns: &[
            "owner_user_id",
            "subject_digest",
            "item_id",
        ...
        order_by: "owner_user_id, subject_digest, item_id, metadata_digest, classifier_profile",
        minimum_schema: 58,
        import_filter: None,
        sealed_columns: &[],
        parent_first: false,
    ...
    table
        ...
        .map(|column| {
                ...
                && schema_version < 65
            {
                ...
                && schema_version < 46
            {
                // A source below v46 kept no per-attempt history, and an empty
...
mod tests {
    ...
    #[test]
    fn pre_v46_fragment_index_jobs_project_an_empty_attempt_history() {
        let table = TABLES
            .iter()
            .find(|table| table.name == "cluster_fragment_index_jobs")
            .copied()
            .expect("fragment-index job table plan");
        let v45 = value_projection(table, 45, false);
        assert!(v45.contains("last_error_code, '', created_at_ms"), "{v45}");
        let current = value_projection(table, SQLITE_SCHEMA_VERSION, false);
        assert!(
