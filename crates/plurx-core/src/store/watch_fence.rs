...
//! local watch read needs a second, per-user condition on top of the bounded
//! permit: this node must have applied the Raft entry that carried the
//! user's latest acknowledged watch write.
//!
//! The fence has two sources, and a local read uses the larger:
//!
...
//!   here for [`WATCH_WRITE_FENCE_TTL`];
//! - the `X-Plurx-Read-After` index a client echoes from an earlier
//!   response's `X-Plurx-Commit-Index`, which covers a write made through
//!   another node.
//!
//! With neither, there is nothing to prove against, so the read goes to
//! Authority. That is the rule the plan states as "a client that does not
//! send the header gets Authority for watch reads on non-writing nodes": a
//! node that holds no write record for the user cannot tell a user who has
//! written nothing from one who wrote through a peer a second ago. A write
//! whose index could not be learned (an older leader, or a proxy, answered
//! it) marks the user unprovable for the same window, which also means
//! Authority. Nothing here reads wall-clock time: the window is monotonic
//! and the fence itself is an index comparison.
//!
...
impl WatchWriteFences {
    ...
    fn required_at(&self, user_id: i64, read_after: Option<u64>, now: Instant) -> Option<u64> {
        ...
        let local = state
            ...
            .map(|(fence, _)| *fence);
        match (local, read_after) {
            (Some(Fence::Unprovable), _) | (None, None) => None,
            (Some(Fence::Index(index)), read_after) => Some(index.max(read_after.unwrap_or(0))),
            (None, Some(read_after)) => Some(read_after),
        }
    }
}
...
mod tests {
    ...
    fn the_larger_of_the_local_write_and_the_echoed_header_is_required() {
        ...
        assert_eq!(fences.required_at(7, None, now), Some(50));
        assert_eq!(fences.required_at(7, Some(40), now), Some(50));
        assert_eq!(fences.required_at(7, Some(60), now), Some(60));
        // Another user's write fences nothing for this one.
        assert_eq!(fences.required_at(8, None, now), None);
        ...
        assert_eq!(fences.required_at(7, None, now), Some(70));
    }
    ...
    fn a_fence_lasts_exactly_one_window_from_its_latest_write() {
        ...
        assert_eq!(fences.required_at(7, None, just_inside), Some(50));
        assert_eq!(
            fences.required_at(7, None, now + WATCH_WRITE_FENCE_TTL),
            None
        );
        ...
        assert_eq!(fences.required_at(8, None, later), Some(80));
    }
    ...
    fn a_full_map_closes_local_reads_instead_of_forgetting_a_live_fence() {
        ...
        assert_eq!(fences.required_at(0, None, now), Some(10));
        let overflow = WATCH_WRITE_FENCE_MAX_USERS as i64;
        fences.record_at(overflow, Some(20), now);
        assert_eq!(fences.required_at(0, None, now), None);
        ...
        assert_eq!(fences.required_at(overflow, None, later), Some(30));
    }
}
