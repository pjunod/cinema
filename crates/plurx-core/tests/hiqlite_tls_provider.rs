...
//! curiosity. That provider used to arrive by accident, through
//! `backup -> s3 -> cryptr/s3` pulling a second `reqwest` whose provider
//! feature Cargo unified onto the first. Removing that edge removed the
//! provider; `http_client::ensure_rustls_crypto_provider` now names `ring`
//! deliberately, at the point of client construction.
//!
...
async fn hiqlite_client_construction_installs_the_ring_crypto_provider() {
    ...
    assert_eq!(
        ...
        "the fork installs `ring`; a different provider here means the process \
         ...
         `install_default_crypto_provider` asks for"
    );
}
