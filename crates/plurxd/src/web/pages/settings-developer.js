...
function seekScratchReservationsCard(){
  ...
  return setCard(`${cardHead("Seek scratch accounting",
      ...
      ${devStaticReq("FFmpeg can write HLS over HTTP","met in the published image",
        "The endpoint needs the engine's <code>http</code> output protocol, which both engines in the published image include. A custom build without network protocols fails every transcode start, with &ldquo;Protocol not found&rdquo; in the server log.","ok")}
