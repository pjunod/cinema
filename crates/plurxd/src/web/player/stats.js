...
function playbackInfoRows(mode,telemetry){
  return PLAYBACK_INFO_FIELDS.filter(field=>
    ...
    .map(field=>{
      ...
      if(value==null&&spec.always) value=["decode_resolution","stream_frame"].includes(field.id)?"Unavailable":
        ["stream_format","device_audio"].includes(field.id)?"Not reported":"—";
...
function playbackInfoOverview(t,live=false){
  ...
  const reason=t.reason||(/cached/i.test(t.method||"")?"Playing a prepared copy. Source and player picture sizes are reported separately.":/direct/i.test(t.method||"")?"The original media is delivered without server conversion.":/remux/i.test(t.method||"")?"Media is repackaged for this player; the video is not re-encoded.":"The server did not provide a conversion reason.");
  return `<div class="pi-state">${esc(t.player_state||"Not reported")}${live?' · Live TV':''}</div><div class="pi-hero">`+
    fact("Source frame",t.source_resolution,t.source_resolution_note||t.source_video,true)+
    fact("Stream frame",t.stream_frame,t.stream_frame_note||"Unavailable")+`</div>`+
    `<div class="pi-explanation">${fact("Player display size",t.decode_resolution,t.decode_resolution_note||"Not reported by this player.")}`+
...
function playbackInfoCompact(t){
  return [
    ["decode_resolution",t.decode_resolution],
    ["stream_frame",t.stream_frame],
    ["player_state",t.player_state],
    ["client_loaded",t.client_loaded]
  ].map(([id,value])=>`<div class="pi-fact"><span class="pi-label">${esc(STATS_ROWS[id].label)}</span><strong>${esc(value||"Unavailable")}</strong>${t[`${id}_note`]?`<p>${esc(t[`${id}_note`])}</p>`:""}</div>`).join("");
}
...
function playbackStatsTelemetry(){
  ...
  return {
    ...
    stream_frame:"Unavailable",stream_frame_note:"No verified encoded output dimensions",
    dynamic_range:range?range.panel:null,dynamic_range_mini:range?range.text:null,
    dynamic_range_note:statsToneMapPeak(h),
    stream_format:level?.videoCodec||"Not reported",
...
function updateStats(){
  ...
  if(overview) overview.innerHTML=STATS_MODE==="mini"
    ?playbackInfoCompact(telemetry):playbackInfoOverview(telemetry);
