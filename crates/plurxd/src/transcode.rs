...
struct Session {
    ...
    cache_location: Option<CachedLocationIdentity>,
...
impl Session {
    ...
    fn compatibility_playlist_published(&self, producer_attempt: u64) -> Option<bool> {
    ...
    #[cfg(any(test, feature = "live-hls-recovery"))]
    async fn kill_child_for_replacement(
        &self,
...
const MAX_PRETRANSCODE_PART_PLAYLIST_BYTES: u64 = 1024 * 1024;
const MAX_RETAINED_PLAYLIST_BYTES: u64 = plurx_core::transcode::manifest::MAX_MANIFEST_BYTES;
const MAX_RETAINED_SEGMENT_DURATION_MS: i64 = 120_000;
...
const MAX_RETAINED_TOTAL_DURATION_MS: i64 = 7 * 24 * 60 * 60 * 1_000;

#[derive(Debug, PartialEq, Eq, serde::Deserialize, serde::Serialize)]
struct PretranscodeStagingIdentity {
    job_id: String,
    file_id: i64,
    source_size: i64,
    source_mtime: i64,
...
async fn read_pretranscode_staging_identity(
...
async fn write_pretranscode_staging_identity(
    temp: &plurx_core::fs_secure::SecureDirectory,
...
async fn bind_pretranscode_staging(
...
const MAX_RETAINED_PART_DIRECTORIES: usize = 10_000;
...
impl TranscodeManager {
    ...
    async fn pretranscode_policy_interruption(
        &self,
        expected: &str,
    ...
    fn pretranscode_target_height_for(
        file: &plurx_core::domain::MediaFile,
    ...
    fn pretranscode_policy_generation_for(
    ...
    async fn encoder_for_file_with_preference(
        &self,
        file: &plurx_core::domain::MediaFile,
        requested: &str,
    ) -> Result<Encoder, String> {
    ...
    async fn start_with_audio_offset(
        ...
        let generation = control.begin_producer_attempt().await.map_err(|reason| {
            format!("rolling control actor rejected the initial producer: {reason:?}")
        })?;
        ...
        if let Err(reason) = control.authorize_producer_install(generation).await {
            let _ = child.kill().await;
            let _ = child.wait().await;
            return Err(format!(
                "rolling control actor rejected initial producer installation: {reason:?}"
            ));
        }
    ...
    async fn downgrade_one_step(
        ...
        let (_replacement, generation) = match session.kill_child_for_replacement().await {
            ...
            tokio::spawn(async move {
                ...
                match outcome {
                    ...
                    copyseg::Outcome::Unsupported(reason) => {
                        ...
                        let (_replacement, generation) = match session
    ...
    async fn register_session(
        ...
        let rejection = {
            // Take the potentially delayed async lock before the actor's final
            // authorization. Publication below is synchronous while both this
            // registry guard and the actor's exact producer fence are held.
            let mut sessions = self.sessions.lock().await;
            ...
            } else if let Err(reason) = session
                .control
                .authorize_producer_install(producer_attempt)
                .await
            {
                Some(SessionRegistrationRejection::Producer(reason))
            } else {
                match session
                    .control
                    .lock_authorized_producer_install(producer_attempt)
    ...
    pub(crate) async fn playlist_with_owner(
        &self,
        session_id: &str,
    ) -> Result<(Vec<u8>, MediaResponseOwner), PlaylistError> {
...
async fn gc_expired_segments(session: &Session) {
    ...
    // waiting, then own the producer transition through both the doomed-name
    // snapshot and every unlink. A replacement that won first makes the
    // sample stale; one that arrives later cannot clear/reseed the directory
    // until these predecessor deletions are finished.
    let producer_attempt = session.control.current_producer_attempt();
    ...
    async fn retention_finishes_predecessor_deletes_before_reused_successor_paths() {
        ...
        *session
        ...
        let replacement = tokio::spawn({
            ...
            let path = dir.path().to_path_buf();
            async move {
                let (_replacement, attempt) = session
                    .kill_child_for_replacement()
                    .await
        ...
        assert!(
            ...
            "replacement must wait while retention owns predecessor paths"
        );

        pause.wait().await;
        gc.await.expect("retention task");
        let successor = replacement.await.expect("replacement task");
        assert_eq!(session.control.current_producer_attempt(), successor);
    ...
    async fn initial_registration_reauthorizes_after_waiting_for_the_registry() {
        ...
        assert!(
            session
                ...
                .as_mut()
                .is_some_and(|child| child.try_wait().is_ok_and(|status| status.is_some())),
            "the unregistered child is synchronously reaped"
        );
    }
    ...
    async fn assert_watchdog_first_copy_replacement(next: WatchNext) {
        ...
        *session
        ...
        let replacement = tokio::spawn(async move {
            let Some(replacement) = replacement_session
                .begin_copy_child_replacement()
                .await
