...
make_interlaced() {
  name="$1" size="$2" source_rate="$3" interlace="$4" parity="$5"
  "$PLURX_FFMPEG" -hide_banner -loglevel error -y \
    -f lavfi -i "testsrc2=size=${size}:rate=${source_rate}:duration=4" \
    -vf "tinterlace=mode=${interlace},setfield=${parity}" -c:v ffv1 "$work/${name}.mkv"
}
...
make_interlaced tff-480 720x480 60000/1001 interleave_top tff
make_interlaced tff-576 720x576 50 interleave_top tff
make_interlaced tff-1080 1920x1080 60000/1001 interleave_top tff
make_interlaced bff-480 720x480 60000/1001 interleave_bottom bff
...
"$PLURX_FFMPEG" -hide_banner -loglevel error -y \
  ...
  -f h264 "$work/progressive-before.h264"
"$PLURX_FFMPEG" -hide_banner -loglevel error -y \
  -f lavfi -i "testsrc2=size=1920x1080:rate=60000/1001:duration=2" \
  -vf "scale=-2:'min(1080,ih)'" -c:v libx264 -preset ultrafast -pix_fmt yuv420p \
  -f h264 "$work/progressive-after.h264"
cmp "$work/progressive-before.h264" "$work/progressive-after.h264"
...
# diverge. The bounded idet pass must overrule this flag before planning, so
# the chosen output omits bwdif and remains identical to the control chain.
"$PLURX_FFMPEG" -hide_banner -loglevel error -y \
  -f lavfi -i "testsrc=size=640x360:rate=30000/1001:duration=10" \
  -vf "setfield=tff" -c:v ffv1 "$work/misflagged.mkv"
summary="$($PLURX_FFPROBE -hide_banner -v info -f lavfi \
  -i "movie=$work/misflagged.mkv:si=0,trim=duration=10,idet" \
  -show_entries frame=media_type -of compact=p=0:nk=1 2>&1 | \
  grep 'Multi frame detection' | tail -1)"
counts="$(printf '%s\n' "$summary" | sed -E \
  's/.*TFF:[[:space:]]*([0-9]+).*BFF:[[:space:]]*([0-9]+).*Progressive:[[:space:]]*([0-9]+).*Undetermined:[[:space:]]*([0-9]+).*/\1 \2 \3 \4/')"
read -r tff bff progressive undetermined <<<"$counts"
total=$((tff + bff + progressive + undetermined))
test "$total" -gt 0
test $((progressive * 100)) -gt $((total * 90))
"$PLURX_FFMPEG" -hide_banner -loglevel error -y -i "$work/misflagged.mkv" \
  -vf "scale=-2:'min(1080,ih)'" -c:v libx264 -preset ultrafast -pix_fmt yuv420p \
  -f h264 "$work/misflagged-planned.h264"
"$PLURX_FFMPEG" -hide_banner -loglevel error -y \
  -f lavfi -i "testsrc=size=640x360:rate=30000/1001:duration=10" \
  -vf "scale=-2:'min(1080,ih)'" -c:v libx264 -preset ultrafast -pix_fmt yuv420p \
  -f h264 "$work/misflagged-control.h264"
psnr="$($PLURX_FFMPEG -hide_banner -i "$work/misflagged-planned.h264" \
  -i "$work/misflagged-control.h264" -filter_complex "[0:v][1:v]psnr" \
