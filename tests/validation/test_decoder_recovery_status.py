...
class DecoderRecoveryStatusContract(unittest.TestCase):
    ...
    def test_m1_review_repair_distinguishes_exact_and_working_tree_evidence(self) -> None:
        self.assertIn(
            f"Exact receipt | `{M1_RECEIPT_HEAD}`: exact mapped history audit 1,384, "
            "catalog 24 points / 30 checks / 1,444 audited files, validation 143/143, "
        ...
        self.assertIn(
            "`d82726ec` retains the exact 3,600-second outer ceiling",
            self.status,
        )
