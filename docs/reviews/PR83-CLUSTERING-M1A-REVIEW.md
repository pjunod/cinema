...
`store/mod.rs` (+2) · `docs/cluster/CLUSTERING-PLAN.md` (+6) · `docs/cluster/PHASE3-SPIKE.md`
...
`docs/cluster/PHASE3-SPIKE.md:102-104` (added by this PR):
...
- **`create_api_key` persisting `"[]"` instead of the caller's scopes.** Scopes
  ...
  `.len() == 1`. This is the surface `docs/cluster/CLUSTERING-PLAN.md:365` designates
...
`docs/cluster/CLUSTERING-PLAN.md:362-364` (added by this PR):
...
Related, and worth correcting in the same edit: `docs/cluster/CLUSTERING-PLAN.md:359`
...
2. **`replicated.rs:292-297`** — scan all ten `sqlite/` modules, not four, and
   ...
   `mod.rs:781`. Or narrow `docs/cluster/PHASE3-SPIKE.md:103` to match what the guard
   does.
...
5. **`docs/cluster/CLUSTERING-PLAN.md:359,363`** — "the extracted suite" → additive
