...
- **`resume_below_seconds` is now vacuous.** It is
  ...
  while `docs/performance/PERF-PLAN.md:476` still claims it lets operators "distinguish
...
`docs/PLAYBACK.md`, `docs/performance/PERF-PLAN.md:471-473`, and the doc comment at
...
frontier advance"), and `docs/performance/PERF-PLAN.md:459` all assert a producer-side
...
- `docs/performance/PERF-REVIEW-RESPONSE.md:270` — "Media-time window 180 s high / 90 s
...
- `docs/clients/APPLE-CLIENT-PARITY.md:12`, `:20` and
  `docs/clients/APPLE-NATIVE-SUBTITLES-PLAN.md:361`, `:365`, `:471` — "current source
  is build 36".
...
- `transcode.rs:4513` / `CHANGELOG.md:467` "never per poll" — `flow_control`
  *is* invoked on every playlist poll (`:4367`); measured, **44 % of holds at
  ...
  (`docs/performance/PERF-PLAN.md:477` phrases it correctly.)
- `docs/performance/PERF-PLAN.md:469` "each below half their limit" — the code releases at
