...
| Developer adaptive Auto enablement | Existing `playback_auto_abr` switch moved to Settings → Developer on `codex/adaptive-developer-advisory`; controller and dated trace/device evidence are advisory only | Integrate into draft #506, then one review and fast lane; refresh dated rows when traces change | F-1 in native adaptive quality build plan and A-05 workboard row |
