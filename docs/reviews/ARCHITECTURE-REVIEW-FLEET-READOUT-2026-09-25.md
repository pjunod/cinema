...
The browser `playback-lab run` observer applied and scored only its first cliff. Commit `36b8f06a5` refuses a multistage browser profile before a costly fixture build, preventing a false two-cliff claim; the device-proxy path retains its stages. Full browser two-cliff scoring remains open. Safari WebDriver session creation timed out and Firefox/geckodriver were unavailable. This is a failed Chrome baseline, not A-04 D3 acceptance.
