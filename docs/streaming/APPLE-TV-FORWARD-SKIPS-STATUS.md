...
make validation-lint                                                        # 2,040 files audited
python3 -m unittest discover -s tests/validation -p 'test_*.py'              # 199 passed, 1 skipped
git diff --check
```
...
Run 2380 then cleared that history audit and reached the ownership inventory.
It correctly required the new test-only response commit, bounded worker poll
sleep and timeout to be classified. `rolling-producer-owners.toml` now records
those three shapes; its focused seven-test inventory, the 2,040-file catalog
