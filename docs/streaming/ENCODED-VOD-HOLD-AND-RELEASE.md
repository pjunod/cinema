...
| 2026-09-21 | gpt-5.6-sol | agent:/root/s01_builder | M3 | [#412](http://192.168.4.7:3000/noirr/plurx/pulls/412) | done: wired the rendition latch, VOD registration wake, rolling-live poll, success-only transition updates, and closed-label generation/termination counters. Production callers that register a live wait use the five-second `QUEUE_WAIT`; VOD admission retries at 250 ms and its one-second policy-read bound registers no capacity waiter, so the conservative one-second stopped-encoder poll remains below every live admission deadline. Focused encoded-stop, VOD handoff, rolling-live yield, latch/failure, status, metrics and restart-continuity checks pass. |
