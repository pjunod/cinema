...
- docs/streaming/VOD-PRESENTATION-PLAN.md, especially decision D6;
- docs/streaming/VOD-M4-HANDOFF.md, especially "Deliberate boundary";
- docs/streaming/VOD-CUTOVER.md;
- this handoff;
...
CI currently runs only `scripts/playback-lab run --suite vod --case
suspend-resume`. Steady playback and seek-storm have separate restoration
prompts in `docs/streaming/VOD-STEADY-ACCEPTANCE-HANDOFF.md` and
`docs/streaming/VOD-SEEK-STORM-ACCEPTANCE-HANDOFF.md`. The bandwidth-cliff case is also
disabled. Its pure shaping, scoring, lifecycle, and artifact contracts continue
to run through `node tests/playback/network-shaping.test.js` on every commit.
