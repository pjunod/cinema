...
| Corrective task PRs | **Active — playback evidence merged; client recovery in exact-head review** | Evidence head `73d9bab7` passed exact-head adversarial review and merged as `7db47b35`. The first client-recovery review rejected nine correctness gaps. Corrections through `fb3424ac` bound startup and established freezes, publish explicit intent before mutation, fence stale recovery, distinguish Auto from Original, coalesce seeks, retain request sequence across replacement, and retire a seek only after a destination frame or advancing audio clock. Android JVM units, the web control contract, pinned-Rust all-target compilation and focused units, 359 Apple simulator tests, iOS/tvOS compilation, and both effort-mode commit gates are green | Pass exact-head adversarial review and the task's one post-fix full-suite run, then merge through Forgejo into the effort |
