...
**Started:** 2026-09-04 · **Effort fork:** `48615baf` · **Latest notified Forgejo main:** `4ce33f95`
...
| Client recovery and intent | **Integrating — [Forgejo #6](http://192.168.4.7:3000/noirr/plurx/pulls/6) remains WIP** | Apple/Android corrections are exact-commit approved. Final web correction `0a70d6fb` passed its tracked effort hook, full static fast lane, and independent source review | Integrate current main and effort, obtain exact-head review and a fresh green development gate |
| Direct-file ranges and pacing | **Open — [Forgejo #14](http://192.168.4.7:3000/noirr/plurx/pulls/14)** | Exact-approved `9239999a` integrates effort `ebe71eb4`; 45 focused stream tests and 121 validation tests passed. Includes a reviewed merge-aware history-audit correction, with no gate bypass | Effort development gate green on the exact current task candidate, then merge into the effort |
| Immutable VOD transcode and burn | **Implementation active** | Separate agent worktree with pinned compiler baseline established; actual producer, rational frame-grid, restart, and real-media decode work is underway | Production transcode/burn recipes pass demand, cancellation, timestamp, initialization, and decoded-output regressions |
