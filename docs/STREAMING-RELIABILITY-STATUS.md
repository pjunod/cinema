...
| Corrective task PRs | **Active — playback evidence first** | Isolated branch `codex/streaming-evidence` repairs pinned Chromium discovery, transition-window scoring, explicit manual/Auto quality operations, and skipped-case refusal | Exact-head adversarial approval, focused contracts, and Effort development gate green |
