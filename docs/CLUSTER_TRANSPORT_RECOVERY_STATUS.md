...
| M3 · recovery budgets | `codex/cluster-transport-m3` | [!60](http://192.168.4.7:3000/noirr/plurx/pulls/60) | final exact-SHA review found the prebuilt-image lane bypassed startup-budget proof; the replacement candidate adds a pull/prove/no-build target and is awaiting exact-SHA review and the effort gate | Virtual-time exact bounds · config/env/Compose precedence |
...
| Operations contracts | pass · 193 tests | The prebuilt-image rollout contract requires pull, single derivation, exact-value proof, and no-build replacement in that order; the complete lane passed with the port-reservation fixture using the normal unsandboxed loopback allowance |
...
| M3 deployment and operations contracts | pass · 193 tests | Compose health timing derives from the three startup phases; the 1,200-second install case requires 2,535 seconds, and the prebuilt-image target proves the exact resolved period before replacement |
...
pull/prove/no-build fleet target and persistent ordering coverage. Real
