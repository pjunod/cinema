...
| M4 · transport status | `codex/cluster-transport-m4` | not opened | every review finding through cache availability and source-time replay is fixed; moved-base focused qualification and exact re-review are pending | Authenticated non-cacheable pre-HTTP status · exact executor-admission identity · independent public/private deadlines · source-time replay aging |
| M5 · recovery campaign | `codex/cluster-transport-m5` | not opened | stable resource slack, late source-archive identity validation, and reset full-attempt counts are corrected on the pre-M4 base; post-M4 rebase and qualification remain | Actual TLS transport matrix · 20 learner and 20 voter cycles |
...
The current M4 integration candidate is based on final M3 PR tip `2c018809`
and includes the observation-identity and response-retention corrections.
Exact combined-tree validation is green and re-review is pending. The local Homebrew
...
| M4 final review correction | pass · focused candidate | The 24-case daemon module, membership web suite, exact operations mapping, all-target check, denied-warning Clippy, formatting, and diff checks pass. Public and private probes settle under independent deadlines; server and UI reject inconsistent nested observers without fallback resurrection; due progress logs cannot be starved; absent or stale caches are not mislabeled as peer-bound omission; and transport ages/deadlines are rebased from the source timestamp with bounded future-clock tolerance |
