...
|---|---|---|---|
| `server.capability-profile` | Reported caps vs named profile | Runtime caps win when any codec/container cap is present; missing fields get conservative browser defaults. Otherwise use the named profile, then `web-h264`. Explicit DV profiles override the legacy all-DV bit, and for a dual-layer source they are the only thing that can approve it. | Rust unit in `http/stream.rs` |
...
| `server.dolby-vision-convert` | Convert Profile 7 to 8.1 | A Profile 7 source with an **HDR10** base, on a client that takes profile 8 but not 7 — every consumer Dolby Vision decoder, since dual-layer never shipped outside Blu-ray hardware — has its RPUs converted to 8.1 in the copy pipe rather than being stripped to HDR10. The base must be HDR10 specifically: 8.1 *means* an HDR10 base, and an HLG base would be 8.4. A client that enumerates 7 gets the stream untouched; one that takes neither gets the ordinary strip. The conversion runs on the far side of the copy's muxer — plurx rewrites the RPU NAL units inside the fragments that one ffmpeg writes — so nothing is re-encoded and no second stream has to be aligned: the verdict stays **remux**, `preserve_dolby_vision` and `convert_dolby_vision` are true together, and the delivered range is `dolby_vision` where the same client on the same title used to be handed `hdr10`. The delivered badge says `dolby_vision` either way, so the wire also carries `delivered_dolby_vision_profile` — the profile the *bytes* hold, 8 for a converted stream and 7 for one preserved for a device that enumerates 7 — which is the only thing separating the two on screen. A verdict that ends up a transcode converts nothing — there is no copy pipe left for the conversion to sit in — and a converted stream is its own fragment-index identity, so a Profile 7 title on a convertible node is indexed three times rather than twice. Settings → Developer → *Convert Dolby Vision Profile 7 to 8.1* turns it off (`playback.dolby_vision_convert`); `0`, `false`, `off` or `no`, in any case, with surrounding whitespace ignored, and anything else — including an unreadable value — leaves it on, because a feature should not disable itself over a typo. The setting is replicated, so the split this used to warn about is gone: the decider and the indexer read the same answer, on every node, and both read it per request rather than at boot. `PLURX_DV_CONVERT` still exists but only seeds that setting once, on a node that has never been told either way; setting it on a node whose setting already exists does nothing. | Client and source matrices in `playback/mod.rs`; the conversion itself in `transcode/dvconvert.rs`, its wiring in `plurxd/dvpipe.rs` |
| `server.dolby-vision-dual-layer` | Blanket DV claim vs enumerated profile | Profiles **7** and **4** are dual-layer — a base layer plus a type-63 enhancement layer, which no consumer decoder outside Blu-ray hardware takes. The legacy blanket "supports Dolby Vision" bit is a claim about the *format*, not about that layer, and every client that ever set it meant single-layer; a dual-layer source is therefore only delivered as Dolby Vision to a client that named the profile **by number**. Everything else is routed to the conversion where the node can build one and to the compatible base layer or a re-encode where it cannot — all of which decode. The rule is asked in three places that must agree, or a stream describes itself honestly and decodes nowhere: which clients may be handed an unconverted copy, what a build sending no readable capability document may be trusted with, and what a copy path with no RPU rewrite in it may serve. The profile answers in both directions: a record naming a single-layer profile is single-layer even when its `el_present_flag` says otherwise, because muxers write that flag over streams that have no enhancement layer and reading it the other way would strip Dolby Vision out of a Profile 5 delivery, which has no compatible base and would tone-map to SDR. `el_present` is consulted only where there is no profile to ask. | `a_blanket_dolby_vision_claim_is_not_a_claim_about_dual_layer` in `playback/mod.rs`; the trust-path clamp in `http/hls.rs`; the served-options guard in `plurxd/transcode.rs` |
...
| `android.required-hls-remux` | Retain a server-required copy transport | `delivery.requires_hls` is part of the effective media recipe. Initial play, seek, audio/offset changes, subtitle changes, recovery, failover, and a committed prepared replacement all route that remux through copy HLS; the prepared successor cannot change the incumbent before commit. An omitted field defaults false, and ordinary remuxes retain progressive delivery. | `androidDvDelivery` JVM transport and recipe-ownership matrices |
...
its current output can present. The v2 clients send the structured document
below; the flat query remains the mixed-fleet fallback and the spelling used by
legacy progressive URLs. Both reach the same `DeviceProfile`, so a file only
transcodes when *this* device genuinely cannot play it — not because a fixed
profile guessed conservatively.
...
Apple omits `max_height` because its four probes do not prove one; it claims
...
The server side of `pgs-v1` is implemented behind Settings → Developer →
*Serve PGS subtitles as an overlay* (`subtitles.pgs_overlay`, off by default;
`PLURX_PGS_OVERLAY` seeds it once and is no longer the gate) and is
...
| Remux, no override | Copy-video HLS; AVPlayer does not take plurx's progressive fMP4 reliably | Progressive `/stream.mp4`, unless `delivery.requires_hls` selects copy-video HLS |
...
Android records the attached transport as part of the effective media recipe.
Initial play, seek, audio/offset changes, subtitle changes, recovery, failover,
and a committed prepared replacement all consult that one value; a successor
does not become authoritative until commit. This keeps a required-HLS remux on
copy HLS without migrating ordinary remuxes away from progressive delivery.
...
- **10-bit sources need the conversion the chain now does.** Live output is SDR
  8-bit H.264 by contract and the software encoder pins `-profile:v high`, so a
  10-bit broadcast reaching libx264 unconverted makes x264 refuse the profile
