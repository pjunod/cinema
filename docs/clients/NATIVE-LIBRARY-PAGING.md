...
BINARY collation, not a locale compare. `resolution` is on the DTO for
movies and home videos only (`ItemKind::carries_resolution`), and the
server's `resolution` sort ranks every other kind — root photos included,
though they have a probed height — at -1, so `resolution ?? -1` is the
server's key for every row. The fixture from §3.1 is loaded by
