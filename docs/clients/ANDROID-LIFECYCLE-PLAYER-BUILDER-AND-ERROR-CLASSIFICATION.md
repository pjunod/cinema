...
| 2026-09-25 | gpt-6-astra | agent:/root/d02_remaining | M8 | `codex/d02-remaining` · `ea85e15f5` | Process-wide offline recovery now replays durable intents on IO and exposes `recovered`; manager calls, preparation, profile restoration, records publication and the boot receiver wait for it. The manager remains paused during replay and API 34+ UIDT handling stays paused afterward. Kotlin compilation passed; queued-download force-stop and startup-time measurements remain owed. |
