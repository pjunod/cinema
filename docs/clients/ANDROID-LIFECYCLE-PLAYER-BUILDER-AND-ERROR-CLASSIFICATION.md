...
| 2026-09-25 | gpt-6-astra | agent:/root/d02_remaining | M8 | `codex/d02-remaining` · `ea85e15f5`/`e57608208` | Process-wide offline recovery now replays durable intents on IO and exposes `recovered`; manager calls, preparation, profile restoration, records publication and the boot receiver wait for it. The manager remains paused during replay and API 34+ UIDT handling stays paused afterward. The follow-up applies the latest durable network policy after replay to close a startup race. Kotlin compilation passed; queued-download force-stop and startup-time measurements remain owed. |
| 2026-09-25 | gpt-6-astra | agent:/root/d02_remaining | M2 | `codex/d02-remaining` · `2b1cee5f4` | Audiobooks use the Audio player role and `WAKE_MODE_NETWORK`; `PlaybackService` owns the existing MediaSession and Media3 notification, with typed foreground permission, runtime notification request and release teardown. The item model's playable kinds are video and audiobook; music has no playable player route yet, so audiobook is the audio-only classification. **Narrow §3 standing-instruction deviation:** prepared commit and rollback rebind the same MediaSession to the active player through `setPlayer`; without those two calls notification controls would target the released predecessor. The owner approved proceeding with that narrow change while absent a user decision. Manifest processing and Kotlin compilation passed; Pixel five-minute background/lock-screen/service teardown acceptance remains owed. |
