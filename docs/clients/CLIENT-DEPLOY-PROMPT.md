...
the fleet with the loop in `docs/streaming/M5-VERIFICATION-PROMPT.md` §1 — note the
...
- **Dolby Vision on a native client** (#865). The rule is not two-way. A
  ...
  the server's decision line for the session — `docs/streaming/M5-VERIFICATION-PROMPT.md`
