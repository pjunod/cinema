...
| P2-10 | `crates/plurxd/src/web/index.html:1256` | "Scan this from the **Plurx** iPhone…" hardcodes the brand in user-facing text; this tree brands as `${APP_NAME}` (the working name at the time) → interpolate APP_NAME. (`.connectqr img` background `#fff` is a deliberate QR quiet zone — keep, comment it) |
...
- **Media badges.** Separate arc, separate plan
  ([MEDIA-BADGES-PLAN.md](../streaming/MEDIA-BADGES-PLAN.md)); the only contact point is
