...
Build 82 uses the canonical no-trailing-slash collection route while the
server accepts the first native spelling during rollout. Build 81 keeps
Library channels separate from the HDHomeRun controller. Every
...
**Proved:** `:app:assembleDebug` compiles build 82 with JDK 25 and the installed
