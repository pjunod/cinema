...
> Status (2026-09-15): source is v0.3.0, Apple build 181. Timer-only
...
> AVPlayer item failures retain their existing transport and compatibility
> ladders. Native text subtitles, the cinematic detail surface, truthful
...
| Playback surface | One `#ploading` element is loading, buffering, failure and prompt at once; 35 call sites paint it and none is told the surface it painted has gone stale | One `PlaybackSurfaceModel` on `PlayerController` replaces `failed`, `playbackError`, `playbackFailureTitle` and `playbackNotice`. Every writer raises a typed fault whose class the shared fixture picks; a blocking surface is only ever drawn over a player its recovery owner has already stopped, and presentation evidence from the periodic observer demotes one that disagrees with a moving picture, logging `surface_disagreement`. Playback debug carries the SURFACE section and the last sixteen faults. The reducer runs all 60 of `tests/playback/playback-surface-contract.json`'s ordered-event cases in `AppleClientTests` ([contract](PLAYBACK-SURFACE-CONTRACT.md)). One thing draws the progress pixel: the staged open is a `client_preparing` fault and `waitingToPlayAtSpecifiedRate` is a `media_waiting` fault, so `buffering` is reachable on this client and the legacy spinner flags are gone. A readiness deadline with a rung left raises row 14 by name; prepared-successor abandonment, a failed control exchange and an unanswered status poll raise `log_only` and draw nothing. A viewer who pauses under a wait retires it (`playback_requested`, ruled 2026-09-13), wired to `wantsPlayback` so the owner's own stop cannot. The notice strip draws the fault's own actions through the same button the full screen uses, so a `refused` change offers the Try again that re-issues it and a demoted fault keeps the recovery it was demoted with | The §6 simulator and device recipes, none of which has run; and row 14's proof is a source-shape pin rather than a behavioural one, because `retryAfterReadinessTimeout` is reachable only over a decoding player |
...
[MEDIA-BADGES-PLAN.md](../streaming/MEDIA-BADGES-PLAN.md) §9. Eligibility is read at render
