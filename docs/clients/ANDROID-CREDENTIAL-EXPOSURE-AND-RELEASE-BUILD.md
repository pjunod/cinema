...
Apple uses its EPUB reader and PDFKit. Android offers PDF only for an
available book no larger than 1 GiB and checks the downloaded byte count
against the selected file. Apple checks the captured profile again after
download and before saving PDF progress. A failed or unsupported read stays
inside the app with an error. No `Session.mediaUrl` or `Session.mediaURL` is handed to
another process.
...
| 2026-09-25 | gpt-6-astra | codex:/root/restore_d03 | M1, M3 API prep | consolidated draft #506 via `codex/d03-grants-and-clients` | M1 server implementation pushed at `29ef06d65`: SQLite v68 and Hiqlite v46 grant tables, import parity, mint/GET/HEAD/revoke routes, 24-hour prune, logout revocation and shared book range delivery. `215667b58` binds each grant to the source login hash so a concurrent mint cannot outlive that login's revocation. Pinned Rust 1.97.1 `cargo check -p plurxd --all-targets --locked` passed. Android and Apple grant API request/response models compile: local `:app:compileDebugKotlin` passed, and `make apple-build` passed iOS and tvOS simulators. The external chooser call sites remain unchanged. Automatic approval review rejected the Android chooser handoff as sensitive egress; the user was asked whether to authorize that exact scoped-URL handoff. No unit or fast-lane tests have run under the requested merge sequence. M5/M6, M9/M10 still require named physical-device evidence. |
