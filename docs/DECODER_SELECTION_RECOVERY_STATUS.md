...
| Task PR | [Forgejo #63](http://192.168.4.7:3000/noirr/plurx/pulls/63), open against the effort; published head `9a491db1` has review changes requested, and the recursive timeout-tree successor is under focused validation. Final exact-head approval and full qualification are pending |
...
| Full PR validation | Exact code head `59d0a4d1` passed `make validate-full`: 23 passed, 0 failed, 2 declared skips for M0. Historical pre-rebase head `01368ce1` remains history only. M1 head `22ab9c8d` completed 22 checks, failed only `cluster-auth`, and declared 2 skips: the 1,800-second aggregate ceiling expired after the replicated Store and live topology contracts passed and while activation was 3/7; the exact activation target then passed 7/7 in 73.83 seconds. `f80100bd` pins that outer validation ceiling to exactly 3,600 seconds but its outer-PGID cleanup missed descendants that created a new group or session; on POSIX, the successor recursively freezes and terminates those descendants before reaping its owned shell. Full-suite requalification is pending after review |
...
| M1 · explicit plan and facts | Recursive timeout-tree repair under focused validation | `07c8f905` makes notification receive one-shot after each fresh readiness poll, bounds bootstrap and steady-state response retries by the shared deadline/stop signal, and makes both steady-response exits mutation-sensitive with observed interruption and branch-specific exit receipts; mandatory pinned-Linux collector regressions pass 36/36. On POSIX, the current successor recursively freezes descendants across process-group/session changes, terminates them, force-kills survivors, and reaps its owned shell while retaining the exact 3,600-second outer budget and unchanged production deadlines |
