...
**Status:** M1 timeout-tree findings implemented; exact-head re-review pending · **Updated:** 2026-09-06 ·
...
| M1 · explicit plan and facts | Timeout-tree repair pending exact-head review | `07c8f905` makes notification receive one-shot after each fresh readiness poll, bounds bootstrap and steady-state response retries by the shared deadline/stop signal, and makes both steady-response exits mutation-sensitive with observed interruption and branch-specific exit receipts; mandatory pinned-Linux collector regressions pass 36/36. `f80100bd` owns, terminates, force-kills, and reaps a timed-out validation process group while retaining the exact 3,600-second outer budget and unchanged production deadlines |
...
| `f80100bd` | Adversarial timeout-tree repair and exact receipt | Findings applied · validation tests pass 124/124, including a SIGTERM-resistant grandchild that cannot survive the outer timeout; the budget is pinned exactly to 3,600 seconds; generated vendor targets are ignored; exact history 1,380 and catalog 24/30/1,443 pass. Its effort hook also passed operations 211/211, formatting, and the pinned all-target workspace compile. This following status-contract update changes no runtime or validation behavior |
