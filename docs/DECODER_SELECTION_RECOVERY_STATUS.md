...
acceptance open · M7b exact candidate under requalification · M8 and promotion
...
| M7b · hardware diagnostic path | Exact candidate under requalification | Backend-aware inventory is implemented. The first eight adversarial findings are closed. The three exact-head findings from `65c688fc` were repaired at `255fd555`; review of that head found three UI/advisory follow-ups: consume the v2 path list, derive the recovery-readiness pill from applied state, and describe selectable-but-unmeasured coverage accurately. Those repairs are committed at `a32ab767`, and current effort head `265e1fc4` is integrated. Exact-tree fast-lane evidence, re-review, one final full suite, draft-PR promotion and merge remain. A real hardware contract is separate fleet evidence |
...
| Focused evidence | Exact `65c688fc`: decoder inventory 15/15, including exact child termination and partial-file removal, plus complete Forgejo effort gate green. Exact `255fd555`: the adversarial reviewer passed the focused inventory and qualification regressions and `git diff --check`. Before current-base integration, the `a32ab767` follow-up passed its selectable-but-unmeasured Rust regression, Developer UI 16/16, status contract 21/21, formatting, diff check and history audit. The merged-base candidate is now being requalified |
| Fast lane | Exact pre-review-repair tree: history 1,601 · catalog 24 points / 33 checks / 1,570 audited files · formatting, locked all-target workspace compile, and changed binary Clippy with warnings denied pass. Forgejo preflight correctly refused `255fd555` because its corrective runtime commit lacked a regression-ledger mapping; `a32ab767` supplies that mapping and passed history with 1,602 corrective commits before current-base integration. Operations had the same 10 failures on this branch and untouched `48ad8716`: the macOS system Bash lacks `mapfile`; 280/290 pass. All-target Clippy reached the same four unused store-fixture constants on both trees |
...
| Next | Re-run the focused/fast lane on the current-effort candidate · obtain exact-head adversarial approval · run one final full suite · mark ready and merge after the effort gate |
...
hardware contract is still required before recovery becomes reachable.
...
- A diagnostic contract qualified against this fleet's *hardware* decoder
  ...
  emits positive selection evidence, but no retained fleet contract covers a
  real hardware path yet, so automatic decode recovery remains unreachable.
  Recorded in full under "M7a" and "M7b".
