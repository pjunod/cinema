...
is on disk and nothing else. Full design record: [HOMEVIDEO-PLAN.md](features/HOMEVIDEO-PLAN.md).
...
- **REQ-HA-1 — Topology: 1 or 3+.** Single node uses the same one-voter replicated path as HA with zero required configuration and stays within the latency, memory, write-rate, and disk-growth budgets in [CLUSTERING-PLAN.md](cluster/CLUSTERING-PLAN.md) §1, measured in M0. HA requires 3+ nodes (odd counts recommended) for raft quorum. **2-node HA is explicitly unsupported** — documented clearly, with the recommendation to add a cheap third voter (Pi/NAS-class) if desired later; no witness process in v1.
