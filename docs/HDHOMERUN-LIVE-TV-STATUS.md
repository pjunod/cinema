...
1. **Finish ATSC 3.0.** The three defects behind the original failure are fixed and merged. What is left is startup latency over the one ATSC 3.0 mux on the test antenna that feeds at all, which carries 58% signal quality and visible decode errors — so the next useful step is a *better signal*, not another constant. Rerun `make live-tv-hardware-check DEVICE=<ipv4> --channel <an ATSC 3.0 number>` after any antenna work, and if a healthy ATSC 3.0 mux still misses the budget, that is real evidence for a further change rather than the marginal-signal noise measured here.
