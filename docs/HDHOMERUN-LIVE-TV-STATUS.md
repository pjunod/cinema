...
**Status:** M5 complete and merged. **ATSC 1.0 plays end to end on a real FLEX 4K.** The hardware pass found three real defects, all fixed and merged; ATSC 3.0 remains unfinished on startup latency over a 58%-quality mux, not on codecs · **Effort:** `effort/hdhomerun-live-tv` ·
...
| 2026-09-05 | the same device and channel, on the fully merged tree | `make live-tv-hardware-check DEVICE=<ipv4> --channel 157.1` | the device now refuses the channel outright: `503 tuner_unavailable`, "the HDHomeRun could not start this channel; a tuner, signal, or authorization may be unavailable". That is the **device's** answer relayed correctly and immediately, not plurx timing out — which is what the budget change was for. It also means that mux is no longer serving at all, having read 58% signal quality with visible decode errors earlier the same day, so ATSC 3.0 on this antenna is blocked on reception rather than on software and no software timing can be measured from it |
| 2026-09-05 | `crates/plurxd/tests/live_tv_two_node.rs` in a container that can bind 80 and 5004 | `cargo test --features cluster-integration-tests --test live_tv_two_node` | **executed for the first time, and all three cases failed — environmentally.** The container binds both ports and sees itself on a private address the runtime accepts, so the fixture device is viable; the daemons then flood `hiqlite ... no such table: watched_outbox` and the cases fail after 300 s. **The control settles it**: the repo's own `cluster_activity` in the same container fails with `daemon readiness timed out` (1 of 2 cases passing), while CI's `cluster_daemon` job on its self-hosted runner is green. So this environment cannot host multi-daemon cluster tests, and that is a fact about the container rather than about the new cases. They still need a host like CI's runner, and are still unproven |
...
1. **Finish ATSC 3.0 — starting with the antenna.** The three defects behind the original failure are fixed and merged. What is left is not software: by the end of the same day the only ATSC 3.0 mux that had been feeding stopped serving entirely and the device answers `tuner_unavailable`, having read 58% signal quality with visible decode errors earlier. Two other ATSC 3.0 channels never delivered a byte. So the next useful step is a *better signal*, not another constant, and no timing measured from this antenna should be treated as a software budget. Rerun `make live-tv-hardware-check DEVICE=<ipv4> --channel <an ATSC 3.0 number>` after any antenna work, and if a healthy ATSC 3.0 mux still misses the budget, that is real evidence for a further change rather than the marginal-signal noise measured here.
