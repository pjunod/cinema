...
| O02 server publication | in progress | rolling segment inventory, VOD materialization manifest, per-session wait pool and status endpoints inventoried | publish anchored readiness and active HTTP wait without adding a scanner or watchdog |
...
| task branches | focused regressions and affected compilation | not run |
