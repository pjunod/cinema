...
[M6-CALLER-HANDOFF.md](M6-CALLER-HANDOFF.md) (where the decision is made) —
this is *the byte-level contract three client teams implement against*, so that
web, Apple and Android can be built in parallel by sessions that never talk to
each other.

**This is the maintained copy — edit it here when the server changes.** Each
platform brief carries a snapshot of it as its own Part II, so a build session
needs exactly one file:
[M6-WEB-CLIENT-BUILD.md](M6-WEB-CLIENT-BUILD.md) ·
[M6-APPLE-CLIENT-BUILD.md](M6-APPLE-CLIENT-BUILD.md) ·
[M6-ANDROID-CLIENT-BUILD.md](M6-ANDROID-CLIENT-BUILD.md). When you change this
file, re-cut those three.

**Standing instruction.** Every type in §4–§9 was copied out of the current
tree with its line number. Re-verify each against the cited file at build time
in case it moved, and if a field's shape disagrees with this document, **the
code wins and this document is the bug** — say so rather than reshaping the
client to match prose.

**The second standing instruction.** Nothing in this contract is yours to
extend. If your platform seems to need a field, a state, or an action name that
is not written here, stop and flag it. All three clients are being built at once
against a server that is already frozen; a client that invents a sixth
acknowledgement state produces a `400` in the field and a merge conflict in the
protocol.
...
## 2. Two gates, and they are independent
...
the action directly (§12) and waits on a measurement, not on more code.

### 2.1 The one thing Gate B changes even when Gate A is false
...
## 3. The exchange, and where it lives
...
## 5. The name trap — read this twice
...
- **`media_origin_ms`** — the source position that the successor's
  session-relative zero maps to. This is the whole alignment mechanism; §8.
- **`effective_selection`** — what the successor will actually deliver, not
  what you asked for. §9. A client that no longer wants it declines by simply
...
validator, so you get no help identifying it. This is why §1's second standing
instruction exists.
...
### 9.1 Which changes can ever be prepared
...
(§13) because the case web failed is not a case the server would have offered
it.
...
### 9.2 The throughput floor, and why `observed_download_bps` is load-bearing
...
## 10. Terminal reasons — 15 values, 3 of them permanent
...
## 11. There is no `commit_replacement` action
...
## 12. Acceptance shared by all three clients
...
## 13. Non-goals — guardrails, each with its reason
...
- **Do not change `dual_player_preparation` in a client brief.** It is a frozen
  ...
  in §14, that is a finding to file, not a literal to edit.
...
## 14. What is measured, what is assumed, and what is open
...
ever prepare are same-codec (§9.1). Line the two tables up:
...
- Behaviour under an active throughput drop. The whole spike ran on a steady
  shaped 80 Mbit/s link. The throughput floor (§9.2) stands in for this and has
