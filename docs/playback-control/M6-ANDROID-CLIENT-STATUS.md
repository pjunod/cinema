...
- On **VOD**: the branch this was built on left `delivered_bps` `None` on a
  VOD session, and the floor needs both numbers, so VOD could not reach a
  preparation there at all. On `main` it can: the P1-3 correction landed, a
  segment answer now carries the meter its own session publishes, and
  `a_segment_answer_carries_the_meter_its_own_session_publishes` is the test
  that keeps it that way. Nothing in this client ever gated on `!isVod`, which
  is why the change costs it nothing — the requirement row names the mechanism
  (the server reports a delivered-throughput number, or it does not) rather
  than the presentation, so it was true on both sides of that change and is
  true now.
...
the web half merged as PR #125; this branch is the third. All three now read the
same server struct in `test_control_wire_conformance`, whose last per-port
discard came out with this change — its own comment said the day Android gained
`ActionAcknowledgement` was the day the discard left with it.
...
corrections, and the two evidence-led fixes (headroom, committed origin). None
of them is on `main`, because that effort is still open. Apple and web reached
`main` on their own, so `main` has had a two-of-three client story since PR
#125, and this branch is the reconciliation.

The policy half moved verbatim. `PreparedReplacement.kt` has no Media3 in it by
design, which is exactly why it ports without argument. What moved is where it
attaches, because `main` restructured the three files it attaches to:

| file | what `main` had changed | how it was resolved |
|---|---|---|
| `Controller.kt` | seeks go through `enqueueSeek`/`executeSeek`, plus `planReplacement` routing | the six abandonment sites keep their *meaning*: a preparation is abandoned wherever the playhead or the session moves, which is now the head of `executeSeek` |
| `PlaybackControlSession.kt` | capture ownership, intent generations, a two-phase subtitle-readiness fence | `main`'s fence is kept whole and this branch's weaker one dropped — it guards the same race with a generation the older one had no equivalent for |
| `PlaybackControlReporter.kt` | state travels as `PlaybackControlCapture`, not a bare snapshot | `settle` takes a capture too, stamped by the session from the same authority `publish` uses. It is taken verbatim rather than reconciled, because its caller has already closed the observation a reconciliation would read through |
