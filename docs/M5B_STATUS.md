...
**Status:** integrated candidate · exact-tree review in progress · **Branch:**
...
| Integration line | Complete | Current `main` at `347d7457` is merged into the effort. M5b follows the intervening timeline, analysis, and staged-generation schemas instead of reusing their version numbers; Apple build 103 is the resulting promotion claim |
...
6. **Do not invent ledger history while importing an old backup.** SQLite v14
   predates the v43 conversion table, so its migration creates an empty ledger;
