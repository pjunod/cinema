...
| Why does every first subtitle wait on one node reading the whole file, while the cluster idles? | [clients/SUBTITLE-CLUSTER-EXTRACTION-PLAN.md](clients/SUBTITLE-CLUSTER-EXTRACTION-PLAN.md) · [review](clients/SUBTITLE-CLUSTER-EXTRACTION-REVIEW.md) |
...
| [SUBTITLE-CLUSTER-EXTRACTION-PLAN.md](clients/SUBTITLE-CLUSTER-EXTRACTION-PLAN.md) | Implementation handoff (v2): why subtitle extraction is a lazy per-node full read the cluster never schedules, the 2026-09-24 fleet evidence, and the build — text tracks ride the index pass in two representations, a `subtitle_source` analysis component with a `foreground` priority any idle node claims (schema v46), per-track publications with portable identity and peer hydration, an off-by-default backfill; three contracts (request lifecycle, publication model, consumer table), six milestones with their acceptance cases. | open |
| [SUBTITLE-CLUSTER-EXTRACTION-REVIEW.md](clients/SUBTITLE-CLUSTER-EXTRACTION-REVIEW.md) | Codex's adversarial review of plan v1: seven blocking findings (request identity, track coverage, ASS burn fidelity, scheduling progress, the client retry ladder, the direct endpoint's pending protocol, portable source identity) and three scope corrections — all accepted into v2. | done |
