...
| Why does every first subtitle wait on one node reading the whole file, while the cluster idles? | [clients/SUBTITLE-CLUSTER-EXTRACTION-PLAN.md](clients/SUBTITLE-CLUSTER-EXTRACTION-PLAN.md) |
...
| [SUBTITLE-CLUSTER-EXTRACTION-PLAN.md](clients/SUBTITLE-CLUSTER-EXTRACTION-PLAN.md) | Why subtitle extraction is a lazy per-node full read the cluster never schedules, the 2026-09-24 fleet evidence, and the plan: text tracks ride the index pass, a `subtitle_source` analysis component any idle node claims, peer hydration, and an off-by-default backfill — all attributable and switchable. | open |
