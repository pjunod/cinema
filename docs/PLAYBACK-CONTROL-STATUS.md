...
| Unit/focused tests | Began only after exact PR-head approval. Passing: actor-mutation-before-reply, early expiry, final-fence rejection, failed predecessor cleanup, retention path reuse, failed-retention unlink, post-admission cancellation, and refresh-owner ABA. Playlist-owner and segment-owner ABA reached their intended cutover but failed because their manager runtime-cache fixture was nested inside the session scratch directory that replacement must empty. The pending test-only correction gives each manager a separate retained tempdir; no production contract changes. |
| Full local gate | Not run yet |
| Hosted CI | PR #615 is open. Corrective runtime commits are mapped; recent hosted attempts also encountered a self-hosted checkout cleanup error outside the PR tree. The exact corrected head is pushed and awaits exact-head approval before hosted recheck. |
