...
| Gate | State |
|---|---|
| Implementation | **Revision in progress** on PR #617 from merged `8c6ccdf7`. Terminal reply insertion, exact-owner route shutdown, lease clamp, pointer removal, and cache-pin removal are one Store transaction. The actor now installs one session-owned End operation before its fallible reply send; exact retries attach to that operation, the reaper observes it immediately, and queued controls cannot mutate after their inherited deadline or a pre-admission cancellation. Durable settlement retries transient/unknown Store outcomes with the same immutable acknowledgement and read-after-unknown. VOD detaches its reader before the acknowledgement becomes visible. Replay telemetry reads the retained platform directly. |
| Adversarial diff review | **Changes requested** at `266b6ad0`: the exact-head reviewer accepted the atomic Store settlement, replay envelope, schema/fixture inventory, and hosted lint repairs, then found the actor-reply ownership gap, a simultaneous-retry/reaper race, one-shot Store persistence, VOD commit-before-cleanup ordering, a global-counter telemetry assertion, and this stale ledger. The working revision addresses all six with actor-synchronous ownership, one shared terminal-result receipt, bounded retry/read-after-unknown, cleanup-before-commit, direct retained-platform evidence, and this update. A new exact-head review is required after commit. |
...
| Hosted CI | Automatic pre-review workflows are diagnostic only. Pre-review runs found and drove fixes for three Clippy findings plus the Store inventory and v14 downgrade fixture. The next pushed exact head will start a fresh diagnostic run; automatic runs are not post-review local evidence. |
| Merge | **Not ready.** |
