...
| Unit/focused tests | Began only after exact PR-head approval. The first focused Rust invocation reached compilation and exposed non-`Send` guard lifetimes, three stale replacement tuple call sites, and two test-module compile defects before any test executed; local repairs await adversarial review before rerun. |
