...
| Adversarial diff review | Reviews 1–7 drove production-build, history-mapping, ordering, cancellation, retention, exact-publication, and false→true→false path-ownership corrections. Later exact-head reviews closed the actor-mutation/reply gap, silent predecessor cleanup failures, detached-garbage accounting, cleanup-worker fan-out and head-of-line blocking, final-rejection causality, and rejected-child termination. The `cc20d906` head was approved before the first full test run; the current completion-gate and full-run test corrections await exact-head approval before rerun. |
| Unit/focused tests | Began only after exact PR-head approval. All ten targeted actor-mutation, expiry, final-fence, scratch-clear, retention, cancellation, and playlist/segment/refresh ABA cases pass. A later full run exposed four additional deterministic cases; their pending repair is described in the full-gate row. |
| Full local gate | `make test` ran after exact-head adversarial approval. Every non-`plurxd` target passed; `plurxd` passed 1,059 of 1,066 tests, ignored 3, and failed four. One startup-gate test repeated the nested manager/session scratch fixture error; one subtitle test asserted a route-qualified URI even though a child media playlist correctly emits relative segment URIs; two copy-replacement watchdog tests exposed that `ChildReplacement::complete(&mut self)` cleared its marker but retained the transition mutex until the caller's scope ended. The pending repair separates the fixture, corrects the relative-URI assertion, and makes completion consume the transaction so its gate is released on return. No failed test has been waived. |
