...
| Gate | State |
|---|---|
| Implementation | **Revision in progress** on PR #617 from merged `8c6ccdf7`. Terminal reply insertion and exact-owner route shutdown are now one Store transaction. Admission/nonterminal work stays under the inherited HTTP deadline; only actor-accepted End transfers to one per-session continuation. The rolling reaper joins a pending terminal-response fence, VOD starts the same durable commit under its lifecycle gate, simultaneous exact retries share one reply, and replay telemetry retains the originally accepted platform even when a later sequence omits capabilities. |
| Adversarial diff review | **Changes requested** at `de32c386`: the reviewer found a crash window between reply insertion and route shutdown, request-wide over-detachment, a rolling reaper race between actor End and response persistence, and missing platform attribution on a capability-free replay. The working revision addresses all four with atomic settlement, the post-linearization continuation, a reaper-visible handoff, and a retained response envelope. A new exact-head review is required after commit. |
...
| Hosted CI | Automatic pre-review workflows are diagnostic only. The latest pushed head `de32c386` passed validation scope and policy/contract preflight but its fast Rust job failed; that log will be folded into this revision before exact-head review. These automatic runs are not post-review local evidence. |
| Merge | **Not ready.** |
