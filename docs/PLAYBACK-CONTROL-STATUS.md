...
`codex/playback-control-m4-deadline-cutoff` — exact-head round 7 at
`f31945a3027dad0d5640972bed16539785f634bd` received three **APPROVE** verdicts.
The authorized focused suites are green: 63 playback-control tests, 2 deferred
flow-signal tests, 1 actor-task-exit test, and 1 producer-signal/retirement test
(67 total). `make check` then stopped at `history-check` because corrective
commit `248d675b` lacked evidence; metadata-only mapping commit `aca0b8d2`
repairs that gap. No rerun has happened, and exact-head round 8 is required
before resuming `make check`.
...
| M4 — server watchdog removal | **In progress** | Contract merged in #618; cutoff-safe deadline-chain ingress and checked legacy-owner inventory merged in #619; round-7 exact-head review approved at `f31945a3027dad0d5640972bed16539785f634bd`; 67 focused tests passed; `make check` stopped at history evidence, repaired by `aca0b8d2` | Obtain exact-head round-8 approval, resume `make check`, and prove the passive cutoff, then add sequenced commands/decisions, operational projection, and the action executor; remove detached recovery loops, in-place fallback, and transition locks/atomics |
...
| Passive deadline/cutoff | **Round-7 runtime approved; 67 focused tests passed; full check paused at history evidence.** Producer facts fold once under transition plus ingress, use fenced publication timestamps, classify non-success exits immediately, preserve progress around bounded sequenced physical-flow barriers, fail closed when the actor/mailbox disappears, serialize actor-task exit fencing with producer transitions, and wait/re-authorize the exact attempt before a full-capacity STOP/CONT syscall. Duplicate pre-drain exits are coalesced and accounted, while stale flow revisions/attempts are rejected. Exact-head round 7 approved the runtime and compile repairs; `make check` stopped before workspace tests because `248d675b` lacked history evidence, and metadata-only mapping `aca0b8d2` repairs it. The result remains provisional until command sequencing lands and emits no recovery action. |
...
| Adversarial implementation review | **Round 7 approved; round 8 required after history repair.** Exact head `f31945a3027dad0d5640972bed16539785f634bd` received three **APPROVE** verdicts. The 67 focused tests passed, then `make check` stopped at `history-check` because `248d675b` lacked corrective evidence. Metadata-only mapping `aca0b8d2` repairs that finding; no rerun or hosted result is claimed. Exact-head round 8 is required next. |
| Unit/focused tests | **67 focused tests passed.** The playback-control suite passed 63 tests, the deferred flow-signal set passed 2, actor-task-exit passed 1, and producer-signal/retirement passed 1. `make check` did not reach workspace tests because history-check stopped first; after `aca0b8d2`, obtain round-8 approval before resuming. |
