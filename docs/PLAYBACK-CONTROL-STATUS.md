...
| Adversarial design review | **Second correction in progress.** The first fourteen P1/P2 findings were resolved in `4c94fd99`. Re-review found four remaining P1 consistency gaps: command-ingress cutoff linearization/due-first dispatch, immediate bounded exit classification, failed physical flow-signal settlement, and a lifecycle deadline/cleanup registry for a live but hung executor. All four are now specified for the next exact-head review. |
