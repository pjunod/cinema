...
| Adversarial diff review | **Changes requested** at exact head `7c8f1a76`: the reviewer accepted the detach-before-settlement ordering, exact receipt-expiry propagation, overlapping rolling retry proof, Arc-cycle removal/drop probe, and validation mapping. It then found that expiry discarded VOD's completed cleanup marker, the cancellation test did not prove its replay reached the cleanup fence, and the boundary evidence omitted late success completion/waiters. The working revision retains the small completed cleanup marker, rendezvous-pauses the replay at the production cleanup wait, proves success completion and delayed waiters fail at the exact boundary, and checks rolling/VOD retention immediately before, at, and after expiry. A new exact-head review is required after commit. |
...
| Hosted CI | Automatic pre-review workflows are diagnostic only. They previously found and drove fixes for Clippy findings plus the Store inventory and v14 downgrade fixture. Current run state remains visible on PR #617 and is not counted as post-review local evidence. |
| Merge | **Not ready.** |
