...
| Adversarial diff review | **Changes requested** at exact head `184487e6`: the reviewer confirmed the earlier Store-I/O cap, retryable receipt, per-waiter replay projection, and real failure seams, then found a cancelled-VOD detach race, fresh retry lives beyond the acknowledgement expiry, a VOD receipt cycle, and a simultaneous-retry test that was actually sequential. The working revision moves every VOD retry behind cleanup, carries the immutable expiry through the retry receipt and both presentation registries, releases expired VOD operation state, breaks the cycle, and pauses the second attempt so two waiters contend deterministically. A new exact-head review is required after commit. |
