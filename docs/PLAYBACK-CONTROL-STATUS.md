...
`codex/playback-control-m4-deadline-cutoff` — review round 4 at exact head
`ba1280bab3114ed49a09799658db345ffd25c834` had primary/concurrency approval,
but the independent contract pass returned **REQUEST CHANGES** for required
history-check mappings; the repair is committed at
`2e2136b27d25a8943e0422a74ca27246cb65119b` and awaits exact-head round 5 before
any tests.
...
| M4 — server watchdog removal | **In progress** | Contract merged in #618; cutoff-safe deadline-chain ingress and checked legacy-owner inventory merged in #619; round-4 history-evidence repairs committed at `2e2136b27d25a8943e0422a74ca27246cb65119b` after the contract pass found missing mappings | Obtain exact-head round-5 approval and prove the passive cutoff, then add sequenced commands/decisions, operational projection, and the action executor; remove detached recovery loops, in-place fallback, and transition locks/atomics |
...
| Passive deadline/cutoff | **Round-4 repairs committed; not tested.** Producer facts fold once under transition plus ingress, use fenced publication timestamps, classify non-success exits immediately, preserve progress around bounded sequenced physical-flow barriers, fail closed when the actor/mailbox disappears, serialize actor-task exit fencing with producer transitions, and wait/re-authorize the exact attempt before a full-capacity STOP/CONT syscall. Duplicate pre-drain exits are coalesced and accounted, while stale flow revisions/attempts are rejected. Corrective runtime commits are now mapped to retained playback/validation evidence and documentation-only repairs are marked ignored by `2e2136b27d25a8943e0422a74ca27246cb65119b`. The result remains provisional until command sequencing lands and emits no recovery action. |
...
| Adversarial implementation review | **Request changes — round 4 contract pass; primary/concurrency approved.** Exact head `ba1280bab3114ed49a09799658db345ffd25c834` was rejected by the independent contract pass because corrective commits `a409a194`, `16723536`, `77d90a8e`, `3827037a`, `728abc86`, and `ba1280ba` lacked required history-check mappings under the post-`e8a910f` enforcement rule, and the handoff sequence omitted `ba1280ba`. Commit `2e2136b27d25a8943e0422a74ca27246cb65119b` maps runtime corrections to retained playback/validation evidence and documentation corrections as ignored non-runtime entries; the parent also repaired the handoff sequence/chronology. Exact-head round 5 is required before tests. |
| Unit/focused tests | **Not run for this slice.** Deterministic tests are authored, including stale revision/attempt and process-level successor, retirement, cleanup, and actor-abort regressions; hosted static preflight skipped test jobs because history-check failed. They wait for exact-head round-5 adversarial approval. |
