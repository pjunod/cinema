...
| Gate | State |
|---|---|
| Implementation contract | **Fourth adversarial correction complete on the exact merged M3 baseline.** It defines deadline policy, cutoff-safe ingress, arm/disarm and event ordering, the one-retry invariant, process-executor ownership, publication-aware cleanup, post-publication proposal behavior, instrumentation, source ownership checks, and the race/failure matrix. |
...
| Adversarial design review | **Fourth correction ready for exact-head re-review.** The first twenty-three findings were addressed through `64e58f86`. The fourth pass found four remaining gaps. This correction preserves the first unconsumed advancing proof across coalescing, gives every failure cleanup an exact publication-aware scratch policy, makes a fixed cleanup-registry permit survive until confirmed reap, distinguishes action failure from the immutable producer decision, and exposes pending-probe plus completion-verification state. No implementation or tests begin until this head is approved. |
