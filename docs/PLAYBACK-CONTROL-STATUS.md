...
| Gate | State |
|---|---|
| Implementation | `fe73efaa`, `fbae4eb9`, `9e0f6c4a`, `af4b8e35`, `2fbb8a4b`, `7c375425`, `3470a0f5`, `298901be`, `15dfea39`, and compile/lifecycle repairs `089cb102`, `b5c43854`, `78e08970`, and `38184342` on `codex/playback-control-m3-delivery-ledger` from merged `5908e838` |
...
| Unit/focused tests | Began only after exact PR-head approval. The first focused Rust invocation reached compilation and exposed non-`Send` guard lifetimes, three stale replacement tuple call sites, and two test-module compile defects before any test executed. The first reviewed repair cleared all but the install-rejection guard lifetime; the detached-reap follow-up was rejected because a failed signal could lose ownership. The current repair lexically ends the exact fence, then owns retry/kill/reap through return and asserts the rejected PID is gone; re-review precedes the next rerun. |
