...
| Static owner inventory | **Interim baseline implemented on the active branch.** `tests/playback/rolling-producer-owners.toml` exact-counts the known recovery/election/replacement owners and scans the whole compatibility source plus every Rust module under `plurxd/src`. Module-wide sentinels cover namespaced, method, and bare task spawns; sleep/sleep-until/interval/interval-at/timeout constructors; timer imports; command construction plus method/UFCS spawn-output-status launches; fork/vfork/clone/clone3/posix-spawn; method/free/UFCS signal-wait-kill-exec forms; and aliases that could hide them. The final §8 check drives every legacy sentinel to zero when deletion lands. |
...
| Adversarial implementation review | **Third pass requested changes at `03ee8ff1`.** The successor and all prior progress/status findings are resolved. The remaining findings map that head in the regression ledger and extend module-wide process coverage to low-level clone/signal/lifecycle, command output/status launch, UFCS, and alias forms. The corrected head requires a fresh exact-head review before tests. |
