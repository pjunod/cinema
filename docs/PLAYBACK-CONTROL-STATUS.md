...
| Static owner inventory | **Interim baseline implemented on the active branch.** `tests/playback/rolling-producer-owners.toml` exact-counts the known recovery/election/replacement owners and scans the whole compatibility source plus every Rust module under `plurxd/src`. Module-wide sentinels cover namespaced, method, and bare task spawns; sleep/sleep-until/interval/interval-at/timeout constructors; command construction plus method/bare-or-namespaced-UFCS spawn-output-status launches; fork/vfork/posix-spawn and process-namespace or bare clone/clone3; method/free/UFCS signal-wait-kill-exec forms; and import, namespace, type, or local-callable aliases that could hide them. The final §8 check drives every legacy sentinel to zero when deletion lands. |
...
| Adversarial implementation review | **Fourth pass requested changes at `8a18a1b5`.** The history binding, successor, and prior progress/status findings are resolved. The remaining findings add bare `Command` UFCS and local task/timer callable aliases, and narrow clone/clone3 coverage to process namespaces plus reviewed bare imports so unrelated `Arc::clone` churn cannot mask a process start. The corrected head requires a fresh exact-head review before tests. |
