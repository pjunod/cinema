...
| Gate | State |
|---|---|
| Implementation | **Revision in progress** on PR #617 from merged `8c6ccdf7`. Terminal reply insertion, exact-owner route shutdown, lease clamp, pointer removal, and cache-pin removal are one Store transaction. The actor installs one session-owned End operation before its fallible reply send; exact retries attach to it, the reaper observes it immediately, and queued controls cannot mutate after their inherited deadline or a pre-admission cancellation. Each settlement attempt bounds every Store write/read/backoff by both one five-second attempt deadline and the exact prepared acknowledgement expiry. Starts, completions, and waiters fail at or after that boundary. Rolling retention derives from the prepared receipt; VOD releases the response/Store graph after expiry but keeps the small owner tombstone. Every VOD outer retry crosses the session-owned reader-detach cleanup before settlement can start. Replay disposition and platform telemetry remain per-request truthful. |
| Adversarial diff review | **Approved** at exact head `09c1bf28` with no P0-P3 findings after three correction rounds. The post-review unit run then exposed two test-harness defects: the VOD cancellation proof pinned the reader lock before status preparation, and the rolling cancellation proof used inadmissible sequence 7 as its first actor command. Both are corrected without changing production semantics; a final exact-head review of those test repairs is required before merge. |
| Unit/focused tests | **Pass.** The exact terminal expiry, VOD cancellation/detach, VOD cleanup identity, rolling contention/retention, and HTTP cancellation/reaper tests pass individually. The complete serialized `plurxd` suite passes with **1,087 passed, 0 failed, 3 ignored**. A sandboxed diagnostic run first produced only loopback-bind/fixture-signal denials; the identical permission-complete run is the counted result. |
| Full local gate | **Pending** final exact-head review, then the remaining workspace/contract gates. |
| Hosted CI | Automatic workflow state remains visible on PR #617. Hosted jobs are supporting evidence only; the complete permission-capable local unit result above is the post-review unit evidence. |
| Merge | **Not ready.** |
