...
5. **Is M3 worth keeping?** (Added 2026-09-24 with M3.) Publication waits
   for the verification probe, whose sample has the cold prefix's own
   ceiling, so a warm start cannot answer before prefix + probe either; on
   the two-node fixture it answered in 3.75 s against 3.78 s cold. What it
   removes is the probe → plan → spawn → encode serialisation after the
   prefix. §6.3's L6 prompt measures that on a real tuner. If the gain is
   under about half a second on 2.1 and the ATSC 3.0 channel, the choice is
   Paul's: keep M3 as is; shorten the verification sample only for
   agreement (a probe of the first second that re-proves the identical
   command publishes, and only a full-ceiling probe may restart), which
   crosses §4's "no heuristic to shorten the prefix" guardrail and needs his
   ruling; or remove M3.
   **Measured locally, 2026-09-25 (PR #496 review response).** The two-node
   fixture's 180p H.264 costs almost nothing to encode, so it cannot answer
   this. The same case was run with the fixture tuner streaming a 1920x1080
   29.97i MPEG-2 + AC-3 transport stream, the 2.1-shaped encode. Over cold, warm saved
   0.55–0.61 s in three runs before the review fixes and 0.60–0.86 s in three runs after them, against 0.06–0.12 s on the 180p fixture. That is at or
   above the half second named here, so **M3 stays**. §6.3's L6 per-channel
   deltas on real tuners (post-merge) confirm or reverse that. Shortening
   the verification sample is still Paul's ruling.
...
|---|---|---|---|---|---|
| 2026-09-23 | claude-opus-5 | https://claude.ai/code/session_01AZemhL7Y1nXGWxUGRC2tkK | M2 (L3) | [PR #455](http://192.168.4.7:3000/noirr/plurx/pulls/455) | Landed. `LiveTvPeers` in `AppState` holds one `PeerTransport` for the process and a five-second, per-owner address memory; the nine `PeerTransport::new` sites in `http/live_tv.rs` are gone and `owner_peer` consults the cache. Invalidation is uniform because it lives inside `LiveTvPeers::request`/`request_stream` rather than at the call sites: every `PeerTransportError` and every internal-path 404 drops the entry. Six focused tests, four of them proved by reverting the change and watching them fail; the PR body lists each revert and the assertion it broke. `plurx_live_tv_owner_peer_total{outcome}` is new. `make live-tv-two-node-check` was not run by this session when M2 landed (it needs a host that can bind 80 and 5004); the PR #455 review then ran it at `6086c8128` in an unprivileged user and network namespace on nuc3 and it passed 4/4, and it passed 4/4 again after the review response (row below). **Not evidence:** no before/after resolution count has been taken on a real ingress. |
| 2026-09-23 | claude-opus-5 | https://claude.ai/code/session_01AZemhL7Y1nXGWxUGRC2tkK | M1 (L2) | [PR #455](http://192.168.4.7:3000/noirr/plurx/pulls/455) | Landed, with one deliberate departure from §3.1 recorded below. `FenceObserver` holds the node's one validated observation; the per-second loop fence validates against it, the start and pre-publication fences keep their direct read and still fail closed. `FENCE_GRACE_MAX_AGE` is 10 s, Paul's decision of 2026-09-23. Six focused tests, four proved by revert (the PR #455 review showed the grace test's keep-on-failure assertion was vacuous; fixed in the review-response row). `plurx_live_tv_fence_observations_total{state}` and `plurx_live_tv_settings_reads_total{site,outcome}` are new (`site` added in the review response). **Departure:** the observation is produced by the existing 1 Hz Live TV settings loop (renamed `settings_observer_loop`) rather than by a second loop that parks on a `Notify` when `held() == 0`. That loop already took exactly this consistent read every second on every node, for the `plurx_live_tv_enabled` and `plurx_live_tv_device_ready` gauges and the guide-cache generation observation, and parking it would blank those gauges after three seconds. So §3.1's "zero reads when none is live" is **not implemented**; the per-node rate is one read per second whether or not anything is live, which is what it was before this change. What the plan set out to remove — the per-session reads — is removed. **Not evidence:** no fleet leader-failover measurement was taken, so the ≈6 s read budget the 10 s grace is sized against is still a budget rather than an observation; §6.3's first GPT prompt is unrun. `make live-tv-two-node-check`: not run when M1 landed; since run, see the M2 row and the review-response row. |
| 2026-09-23 | claude-opus-5 | https://claude.ai/code/session_01AZemhL7Y1nXGWxUGRC2tkK | M3 (L6) | — | **Not implemented.** Every acceptance case in §5.3 except `plan_inputs_agree_covers_every_field_the_planner_reads` runs on the `cluster-integration-tests` two-node harness, whose fixture device streams a real ffmpeg-generated TS on port 5004 and needs a host that can bind 80. This session could not run that harness, and a warm start that spawns FFmpeg before its plan has been re-proven from the bytes of this tune is exactly the change that must not be merged on a pure-function test alone. Left whole for a session that has the harness. |
| 2026-09-23 | claude-opus-5 | https://claude.ai/code/session_01AZemhL7Y1nXGWxUGRC2tkK | M4 (L9) | — | **Not implemented.** The orphan owner is the load-bearing part — a failed cleanup today leaves a session counted against `MAX_TERMINAL_TOMBSTONES` for ever, so 256 of them refuse every start — and its §5.4 cases need a fake FFmpeg that ignores SIGTERM for eight seconds, which is the same harness M3 needs. The incarnation file and the blocking inventory are separable and provable here; they were left with the rest rather than split off, because a half-landed M4 would leave the board saying L9 was addressed when the capacity leak it names is still open. |
| 2026-09-24 | claude-opus-5-5 | https://claude.ai/code/session_01AZemhL7Y1nXGWxUGRC2tkK | M1/M2 review response | [PR #455](http://192.168.4.7:3000/noirr/plurx/pulls/455) | Answers the review's two P2 findings; main merged into the branch first (clean). (1) No M1 test had ever made a settings read fail, so the keep-the-last-observation arm of `observe_fence` was unpinned. `LiveTvManager` now reads settings through one `settings_snapshot` seam with a `#[cfg(test)]` injected failure, and `a_read_that_keeps_failing_is_graced_then_ends_the_session_naming_the_settings_store` takes a real failing observer read every paused second: the t=0 observation is kept (same `Arc`), `site="observer",outcome="failed"` counts each failure, the session survives at t=9 s, ends at t=11 s naming the replicated settings, and the first good read is fresh again. Proved: the reviewer's mutant (`send_replace(None)` in the `Err` arm) fails it. (2a) `plurx_live_tv_settings_reads_total` was documented as the observer's 1/s plus one per start, but every Live TV handler reads through `config()` too. It now has `site` = `observer` / `fence` / `request`; the fence's read rate is `observer` + `fence`. `settings_reads_are_counted_by_the_site_that_took_them` pins it; proved by routing the observer read back through `config()`. (2b) `OWNER_PEER_TTL`'s comment, its guard test and the M2 regressions entry said the memory drops a silent peer before the roster does; it cannot, because it refills from the roster. They now say the TTL bounds the staleness the memory adds (at most 5 s over the 30 s window) and that the first failed exchange is what drops a dead owner. Comment only. `make live-tv-two-node-check` passed 4/4 after these changes (423 s), run on nuc3 inside `unshare -rnm` with a dummy `10.77.0.1/24` interface and default route so the fixture can bind 80 and 5004, with the built `plurxd` bind-mounted over the shared target path. That also means the two-node harness M3 and M4 were left for **is** runnable without root on nuc3; they remain not implemented, out of scope for a review response. |
| 2026-09-24 | claude-opus-5-5 | https://claude.ai/code/session_01AZemhL7Y1nXGWxUGRC2tkK | M3 (L6) | [PR #496](http://192.168.4.7:3000/noirr/plurx/pulls/496) | **Implemented, on the L-03 shared transport.** #482 moved the tuner GET, the prefix and the probe out of `run_live_session_inner` and into `DvrTransport` (§2.3's serial sequence no longer exists in the session), so the warm path is a transport mode rather than a `LiveTunerInput` tee. Re-verified against `7939a3f1e`: `run_transport` (`live_tv/dvr.rs`) collects the prefix and probes it; `pump_tuner_fanout` holds that prefix for the first consumer; a joiner plans from the transport's facts while fresh (`source_for_viewer`). As built: `LiveTvManager::source_facts` caches the full `LiveSourceFacts` a session planned from under `SourceFormatKey` with `source_format_expiry` (20 min, clipped to the programme end); cold openers and joiners record, a mux change or a contradicted input forgets. An opener whose request resolves a plan from unexpired facts opens the transport warm (`DvrTransport::open_warm`): the fan-out keeps no prefix, feeds the opener from the tuner's first byte, and collects those first bytes as sample epoch 1 beside the feed, probed exactly as a re-probe is. The session plans from the cached facts, takes admission, spawns FFmpeg and attaches at once; publication waits for `opening_facts()`. **Agreement is identity, not a field list (departure from §3.3's `PlanInputs`):** the probed facts must resolve a delivery identical to the running one apart from the facts it carries, the FFmpeg argument vector built from it with the same admission must be identical byte for byte, and on an encode route the admission inputs (height, codec, HDR) must match. A field list must be kept in step with the planner and the command builder by hand; this runs them. The test shows the difference: under the legacy AAC profile an AC-3 5.1 → E-AC-3 stereo change produces the same command and agrees, where a field list would restart. On disagreement the FFmpeg is killed and reaped (an unconfirmed exit goes to the session's cleanup), the viewer keeps its seat, the scratch is emptied, the facts are replaced, and the start re-plans from the probed facts inside the same `session.started` budget; it then attaches at the live edge as a joiner does, and #482's input check applies. Other departures: the facts cache is its own map, not a field on `CachedSourceFormat` (a stderr format change replaces that record and must drop the facts, not merge them); `plurx_live_tv_start_plan_total{source}` has a fourth value, `joined`; FFmpeg is spawned as soon as the plan is made rather than on the first chunk (the transport holds the stream until the opener attaches, so the first byte still reaches it); the plan's 2.5 s FFprobe wrapper would exceed `SOURCE_PROBE_TIME` (2 s) and fail rather than be slow, so the fixture's probe sleeps 1 s. Tests (`live_tv::tests`, through the real start path, transport and fan-out, with a loopback HDHomeRun and scripted FFmpeg/FFprobe): `a_warm_start_spawns_before_the_prefix_ceiling_and_publishes_only_after_agreement`, `a_warm_start_that_disagrees_restarts_cold_and_never_publishes_the_wrong_plan`, `a_cold_start_probes_before_it_spawns_and_caches_the_facts`, `every_tuner_byte_reaches_ffmpeg_exactly_once_on_a_warm_start`, `an_expired_or_foreign_generation_entry_is_cold`, `warm_agreement_is_an_identical_delivery_and_command`; and in `tests/live_tv_two_node.rs` `a_second_watch_starts_warm_and_the_real_probe_agrees` (real FFmpeg and FFprobe). Revert proofs, each exit 101: no publication gate → published before the probe; a transport that holds the prefix → the warm FFmpeg is not fed while verifying; an agreement check that always agrees → two spawns, not three; cold starts not recording facts → no warm start. `make live-tv-two-node-check`: see the evidence row below. **Finding — the saving is small, and the gate is why.** Publication waits for the verification probe, and the verification sample is the same `SOURCE_PREFIX_TIME` / `SOURCE_PREFIX_BYTES` ceiling the cold prefix was, so a warm start cannot publish before prefix + probe either. On a real-time source the cold path loses little: its prefix is fed to FFmpeg in one burst, so FFmpeg has 3 s of media the moment it starts. On the two-node fixture (real-time 320x180 H.264) the cold start answered in 3.78 s and the warm one in 3.75 s. The remaining gain is the probe → plan → spawn → encode-burst serialisation, which depends on the encode route and the probe time; the §3.3 claim that the first segment falls "by the prefix time" does not hold under its own publication rule. §6.3's L6 prompt is what measures it on a tuner; §7 question 5 records the choice. **Not evidence:** no fleet or real-tuner measurement. |
| 2026-09-24 | claude-opus-5-5 | https://claude.ai/code/session_01AZemhL7Y1nXGWxUGRC2tkK | M4 (L9) | [PR #496](http://192.168.4.7:3000/noirr/plurx/pulls/496) | **Implemented.** Re-verified against `7939a3f1e`: `run_live_session` returned before `retire_session` on a cleanup error; the registry check was `sessions.len() + terminals.len() >= MAX_TERMINAL_TOMBSTONES`; `cancel_and_wait` retried a failed cleanup itself; `inspect_scratch` used `tokio::fs` per entry and was awaited inline by the tick; `session()` answered "live-TV capability expired". As built: `run_live_session` always retires, and first moves what a failed cleanup left — the unconfirmed child with its job and encoder admission, or the directory — into an `OrphanSession` on `LiveTvManager::orphans` (a tokio mutex), with the directory in `scratch_claims` so the hourly sweep leaves it. `scratch_sweep_loop` now wakes every `ORPHAN_RETRY` (5 s): it runs the hourly sweep when due, retries each orphan (`start_kill` + `try_wait`, then the directory), and touches the incarnation. `ORPHAN_MAX` (64) drops the oldest scratch-only orphan; a process orphan is never dropped. Drains and shutdown still acknowledge only when the orphans of the sessions they cancelled — and, for `drain_before`, every orphan of an older generation; for shutdown, every orphan — are gone, under the same `SESSION_DRAIN_TIMEOUT`, so a drain proof still means the child is gone. `plurx_live_tv_orphans{kind}` is the gauge. The inventory is one `spawn_blocking` walk (`scan_scratch_directory`) with every check kept, plus the bounded playlist read; the tick keeps at most one scan in flight (`inventory_scan`) and waits for it at most one `SESSION_TICK`. The incarnation file is `<runtime cache>/live-tv/incarnation.json`, beside `guide.json`; an unknown capability says "restarted" when the previous process was seen within `PROVISIONAL_TIMEOUT + CAPABILITY_IDLE_TIMEOUT + 30 s` (115 s), otherwise "not available"; the code stays `capability_expired`. (A tombstone-pruning defect first claimed here was withdrawn after the PR #496 review: `request_session` already pruned expired tombstones in the start's own lock hold. See the review-response row.) Departures: `start_kill` is SIGKILL, which no process can ignore, so the plan's "fake FFmpeg ignores SIGTERM for 8 s" cannot produce an unconfirmed exit, and a process in uninterruptible sleep cannot be made unprivileged; a `#[cfg(test)]` seam withholds the reap, with a real child that the orphan owner then kills and reaps. A hung scan is a `#[cfg(test)]` seam too. The plan's count of `spawn_blocking` dispatches per scan is not measured; the walk is one closure by construction. Tests: `a_failed_cleanup_retires_the_session_and_keeps_the_child_until_reaped`, `a_failed_scratch_cleanup_retires_the_session_and_a_drain_waits_for_the_directory`, `two_hundred_failed_cleanups_do_not_refuse_starts` (300), `inspect_scratch_keeps_every_check_in_one_blocking_scan` (seven rules, including a 200 MiB growing `.tmp` under an unchanged playlist), `a_hung_inventory_does_not_stack_and_the_progress_timer_still_ends_the_session` (on a published session: a start still waiting is timed out by its own waiter, outside the tick), `an_unknown_capability_is_unavailable_unless_a_recent_incarnation_proves_a_restart`, `the_incarnation_is_touched_only_while_something_is_live`. Revert proofs, each exit 101: no retirement on failed cleanup (three tests), no scratch claim (the sweep deletes the live child's directory), the inline scan await (the session never ends), the old lookup text. The web, Apple and Android clients render `capability_expired` with their own text, so the new sentence reaches the API body and the owner log, not a client toast; the §6.3 L9 prompt's last step is restated in the row below. **Not evidence:** §6.3's L9 prompt is unrun. |
| 2026-09-24 | claude-opus-5-5 | https://claude.ai/code/session_01AZemhL7Y1nXGWxUGRC2tkK | M3/M4 gates and post-merge steps | [PR #496](http://192.168.4.7:3000/noirr/plurx/pulls/496) | **Gates.** `make live-tv-two-node-check`: 6/6 passed in 639 s, exit 0, on nuc3 inside `unshare -rnm` with a dummy `10.77.0.1/24` interface and default route, the built `plurxd` bind-mounted over the shared target path, built at `8275bc8d4` (every later commit on the branch changes only unit tests, the ownership ledger, docs, or merges main without touching a Live TV file). The new case, `a_second_watch_starts_warm_and_the_real_probe_agrees`, answered the cold watch in 3.78 s and the warm one in 3.75 s. The PR body lists the fast-lane gates at the final head with exit codes. **GPT steps (post-merge, fleet).** (1) L6: §6.3's L6 prompt as written, and additionally report `plurx_live_tv_start_plan_total{source}` on `media1` before and after each run (expect `warm_agreed` to rise on each warm watch and `warm_disagreed` to stay 0) and, per run, the owner-log instants of the tuner GET, the FFmpeg spawn and the first `Provisional`; the number that answers open question 5 is warm first-`Provisional` minus cold first-`Provisional`, per channel, not averaged. (2) L9: §6.3's L9 prompt, with its last step replaced by: restart plurxd on the owner while a session is live; within 90 s run `curl -s -o /dev/stdout -w ' %{http_code}' -H 'Authorization: Bearer <admin token>' https://<owner>/api/v1/live-tv/sessions/<stale capability>/index.m3u8` and expect `410`, code `capability_expired`, and a message containing "restarted"; repeat after 3 minutes and expect "not available". The web toast shows its own text for that code by design; report it but do not expect the sentence there. Also report `plurx_live_tv_orphans{kind}` through the `chattr +i` step: `scratch` 1 while the directory is immutable, 0 within 10 s of `chattr -i`. |
| 2026-09-25 | claude-opus-5-5 | https://claude.ai/code/session_01AZemhL7Y1nXGWxUGRC2tkK | M3/M4 review response | [PR #496](http://192.168.4.7:3000/noirr/plurx/pulls/496) | Answers the single adversarial review ([comment 4468](http://192.168.4.7:3000/noirr/plurx/pulls/496#issuecomment-4468)). Main was merged first (`444cad1fc`, #505's FFmpeg write boundary and 20 other commits). The one conflict was the ownership ledger. Its three shared rows were re-measured on the merged tree, and the tree was built and tested before any fix (2798 passed). (1) **Fixed: a warm opening stranded its joiners.** The fan-out began a warm opening's sample only after a consumer attached. A joiner cannot attach before that sample's probe, and its pending seat kept the transport from retiring. So an opener that ended before attaching (stopped while it waited for an encoder slot, refused admission, or a spawn or attach failure) left every joiner hanging to its start deadline, which then failed with "no signal". `DvrTransport` now records the warm opener (`open_warm(opener)`), and the session marks it done when it attaches or releases its seat (`warm_opener_done`). `await_first_consumer` stops waiting once a warm opener is gone without attaching while another consumer holds a seat. The fan-out then collects and probes the opening sample with nobody attached, as a cold transport probes its prefix, and the joiner plans from it (`joined`). The reviewer's reproduction is now `a_joiner_of_a_warm_opening_starts_when_the_opener_leaves_before_attaching`. Proved: with the early return removed, the joiner fails after 13.0 s with `StartupTimeout`. (2) **Fixed: the orphan owner held its list across `remove_dir_all`,** and `drain_orphans` took the list with no deadline. So one stalled mount held every stop, drain and shutdown past `SESSION_DRAIN_TIMEOUT`, and held a failed cleanup's handoff before `retire_session`. Each orphan's removal now runs on its own task (`OrphanSession::removal`), one in flight, kept rather than replaced while it hangs. `retry_orphan` never waits, and `drain_orphans` bounds its wait for the list by its deadline. Only from here does the M4 row's "under the same `SESSION_DRAIN_TIMEOUT`" hold on a stalled filesystem. Pinned by `a_hung_orphan_removal_holds_up_no_handoff_and_no_drain_past_its_deadline`, which uses a `#[cfg(test)]` switch that makes every orphan removal hang. Proved: with the removal awaited inline under the lock again, the retry pass never returns. (3) **Withdrawn: the tombstone defect the M4 row reported did not exist.** `request_session`, called in the same lock hold just before the capacity check, pruned expired tombstones at the merge-base too. The redundant prune in `recovery_history_admits` is removed. `a_full_recovery_history_admits_again_once_its_minute_is_up`, which called the helper directly, is replaced by `a_full_recovery_history_refuses_a_start_until_its_tombstones_expire` through `start_local`. `two_hundred_failed_cleanups_do_not_refuse_starts` no longer calls the helper. The claim is struck from the M4 row, the PR body and `validation/regressions.d/025f5c73-…`. The commit stays corrective for its two real defects. (4) **Measured: open question 5.** The case was run on nuc3 with the real FFmpeg and FFprobe. Its fixture tuner streamed a 30 s 1920x1080 29.97i MPEG-2 (12 Mb/s, `+ilme+ildct`, top field first, B-frames) + AC-3 transport stream with `-re -stream_loop -1 -c copy`, generated as L-03's caption-audit fixture is (a local harness change, not committed). Two batches of three runs. Before the review fixes: cold 4.28 / 4.37 / 4.36 s, warm 3.73 / 3.77 / 3.75 s, warm saves 0.55 / 0.60 / 0.61 s. After them, at the final code: cold 4.40 / 4.45 / 4.62 s, warm 3.80 / 3.83 / 3.76 s, warm saves 0.60 / 0.62 / 0.86 s. The 180p control saved 0.06 s and then 0.12 s. The route is an encode on both watches: MPEG-2 1080i to H.264 1280x720 at 59.94 fps with field-rate deinterlacing, and AC-3 to AAC. Which encoder the node chose is not in the start response and was not checked. **Decision: M3 stays.** The fleet L6 deltas are post-merge and still decide it. (5) `input_contradicts_plan`'s doc comment is back on its function. **Gates** at `c2afe63b7`: fmt, clippy `-D warnings`, and `cargo test -p plurxd` 2800 passed, 0 failed, 13 ignored, all exit 0. `history-check`, `validation-lint`, the validation unittests (242), `operations-check` (503) and `spike-lock-check` all exit 0. `make live-tv-two-node-check`: 6/6 passed in 640 s, exit 0 (its warm case: cold 3.83 s, warm 3.68 s), inside `unshare -rnm` as before. **Not evidence:** no fleet or real-tuner run. |
| 2026-09-25 | claude-opus-5-5 | https://claude.ai/code/session_01AZemhL7Y1nXGWxUGRC2tkK | Promotion merge of main | [PR #496](http://192.168.4.7:3000/noirr/plurx/pulls/496) | Merged `origin/main` at `8251d14f7` (`d878e4942`). Main's caption-proof work met the warm-start loop in `run_live_session_inner`: it is carried over through `advertise_proven_captions`, applied to the delivery that is published and not to the plan FFmpeg is built from, so `warm_plan_agrees` still compares plans and a warm start that agrees publishes the probed delivery with the same `captions_advertised` reason. Ownership rows re-measured by zeroing: `namespaced-task-spawn` 622, `namespaced-time-constructor` 966, `process-command-construction` 173, `process-lifecycle-method` 395. `cargo check -p plurxd --tests --locked`, `cargo fmt --check`, `make history-check`, `make validation-lint`, the validation unittests and `make operations-check` exit 0; `cargo test -p plurxd --bin plurxd live_tv` 242 passed, 0 failed, exit 0. |
