...
**Status:** executing on `effort/optical-media`; M0–M5 foundations, managed
owner-routed VOD and web playback surfaces are in progress; no physical hardware acceptance exists · **Written:**
...
| M3 playback/API | partial | Authenticated drive/disc/title reads, executable managed-source decisions, local session start, session-bound progress, admin match and generation-checked eject are implemented without device paths. Commit `07402331` registers managed encoded titles in the existing immutable VOD controller without fictitious file/item IDs; the exact drive lease survives until terminal reader/process cleanup, active request IDs replay idempotently, conflicts fail typed, and a revoked insertion/lease terminates its producer. Commits `eeee9dfe`, `9a2a7b5b` and `59f52278` complete the browser decision/start/progress envelope, bound timeline, track and resource inputs, and publish owner-routed optical sessions through the existing durable HLS route ledger. Copy indexing and the physical seek proof remain open; no release claim is made. |
| M4 cluster | partial | Protocol 7 advertises bounded path-free drive ownership and remote discovery. Commit `a59d1429` adds bounded exact-auth owner forwarding and signed responses for remote disc reads, decisions and eject. Commit `59f52278` adds owner-only start/progress forwarding and durable route publication, reusing the authenticated playlist, segment, status and stop relay while explicitly declining physical-source adoption; [M4 evidence](../evidence/OPTICAL-M4-2026-09-21.md). Live multi-node and owner-loss playback acceptance remain open. |
| M5 web | partial | Commit `9a2a7b5b` adds a finite path-free source adapter to the existing player, an authorized Home shelf, conditional Discs navigation, drive/disc/title routes, chapter starts, track facts, resume/play, progress, polling and admin eject across all three layouts. Commit `ae003ad1` adds verified current-match display, existing-item search, explicit match-kind confirmation, save and unmatch. Focused browser evidence, screenshots and physical playback remain open. |
| M6 native | partial | Commit `09cf7414` adds path-free optical models and APIs, authorized Home discovery, disc/title navigation, resume, chapters and track selection on Apple and Android. Commits `1b63b7e9` and `019d62b3` remove the interim players and adapt optical playback into each established full controller without fake catalog IDs, preserving session replacement, seek, progress, teardown, Picture in Picture, TV input, telemetry and typed terminal surfaces. Build 174 compiles for iOS/tvOS and build 112 assembles for Android; [M6 evidence](../evidence/OPTICAL-M6-2026-09-21.md). Physical focus, track-change, seek, resume, Picture in Picture and device acceptance remain open. |
