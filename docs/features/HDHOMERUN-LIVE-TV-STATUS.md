...
| Forgejo main synchronization | merged | [#7](http://forge.lan:3000/noirr/plurx/pulls/7) | Astra approved `c4b66e1d`; fresh gate run 61 passed; merge `e8095916` |
| M2 live HLS and cluster relay | merged | [#13](http://forge.lan:3000/noirr/plurx/pulls/13) | Astra approved `fc164045`; exact pinned fmt/check/clippy and 47 focused tests passed; effort gate run 85 passed; merge `f2773ca8` |
...
| M4 Apple | merged | [#17](http://forge.lan:3000/noirr/plurx/pulls/17) | Astra approved `8cfe8cc5`; iOS/tvOS compile and 14 focused tests pass on each simulator; effort gate 95 passed; merge `fc9c8f45` |
| M4 Android | reviewed; effort PR open and green | [#23](http://forge.lan:3000/noirr/plurx/pulls/23) | dedicated live player and Developer controls; 15 focused tests and both instrumentation tests pass on an API 36 phone emulator; exact JDK 25 / SDK 37 build passes; adversarial review closed a shared critical double-allocation path. Television instrumentation is still outstanding |
| M5 endless-source acceptance | merged | [#28](http://forge.lan:3000/noirr/plurx/pulls/28) | `scripts/live-tv-endless` drives ten real window rollovers against an endless FFmpeg source: one tuner GET, window never past six, 29.9 MB peak scratch, orphan expiry in 65 s, zero bytes left |
| M5 carried-forward review closeout | merged | [#30](http://forge.lan:3000/noirr/plurx/pulls/30) | 25 web tests, 16 Android unit tests, 4 instrumentation tests (2 `LiveTvUiTest`, 2 `LiveTvFileBarrierStoreTest`) pass |
| M5 two-node acceptance | merged and **passing** | [#31](http://forge.lan:3000/noirr/plurx/pulls/31) | three cases behind `cluster-integration-tests`; `make live-tv-two-node-check`. All three pass in 370 s in a container that can bind 80 and 5004. Getting there took two fixes, one of them a real product defect — see the table below |
| M5 hardware acceptance | **run, and passed for ATSC 1.0** | [#32](http://forge.lan:3000/noirr/plurx/pulls/32) | real FLEX 4K over an antenna: 6.4 s to a fetchable segment of a 15 s budget, 4.004 s segments, 1080i MPEG-2/AC-3 in and H.264 720p/AAC out, 2 sessions then `tuner_capacity`, the device itself confirming the tuners held and freed, 0 bytes left. One defect found: ATSC 3.0 always `startup_timeout` |
| M5 documentation | merged | [#33](http://forge.lan:3000/noirr/plurx/pulls/33) | Live TV now appears in FEATURES §4a, ROADMAP, ARCHITECTURE §3a, PLAYBACK, OPERATIONS (runbook + problems + ports), CHEATSHEET §5, SECURITY, both client-parity docs, REQUIREMENTS §3a and the README, whose live-TV non-goal is narrowed to "no DVR" rather than deleted |
| M5 defects found by hardware | **all three fixed and merged** | [#39](http://forge.lan:3000/noirr/plurx/pulls/39) · [#40](http://forge.lan:3000/noirr/plurx/pulls/40) · [#42](http://forge.lan:3000/noirr/plurx/pulls/42) | a 10-bit source meeting a pinned 8-bit profile, a flat startup budget that gave one message to two unrelated situations, and an 8 MiB probesize spent on tuner stream. Each with coverage executed against the unfixed implementation |
| M5 promotion | ready | — | the household numbers are measured and in the documentation; ATSC 1.0, unprotected ATSC 3.0, and the two-node cases all pass. What is still open needs hardware this agent has no access to: Android television instrumentation and physical client playback |
...
| 2026-09-04 | physical tuner discovery | sanitized `discover.json`, no tuner stream opened | FLEX 4K, HDFX-4K, device `1040A1B2`, four tuners, firmware `20260326`; not playback proof |
...
| 2026-09-05 | the same device, chasing channel 157.1 (ATSC 3.0, HEVC Main 10 1080 + AC-3) | three separate diagnoses | **three real defects, all fixed and merged.** (1) The live filter chain pinned no pixel format, so a 10-bit broadcast reached libx264 — which is handed `-profile:v high` — and x264 refused with "high profile doesn't support a bit depth of 10", so FFmpeg exited before publishing. Any 10-bit live source was affected. Reproduced deterministically from a 4-second capture, with and without the conversion, plus the same pair against an 8-bit MPEG-2 capture to show the conversion costs that path nothing. Fixed in [#39](http://forge.lan:3000/noirr/plurx/pulls/39). (2) `STARTUP_TIMEOUT` was flat, so it gave one sentence to a channel quietly working and to two ATSC 3.0 channels that accept the connection and deliver **zero** bytes — and it expired before the encoder error above could be reported at all. Fixed in [#40](http://forge.lan:3000/noirr/plurx/pulls/40); extending it is how the encoder error was found, as `stream_failed: FFmpeg exited with exit status: 1`. (3) `-probesize` was 8 MiB of *tuner stream*: measured 7.06 s to first segment against 5.55 s at 2 MiB and 5.54 s at 512 KiB on the antenna's strongest channel, and roughly 23 s of wall time on its own on the 2.8 Mbps ATSC 3.0 mux. Fixed in [#42](http://forge.lan:3000/noirr/plurx/pulls/42) |
...
| 2026-09-05 | `crates/plurxd/tests/live_tv_two_node.rs` on **two** lab hosts — lab3, and media1 (16 cores, 60 GB) in the pinned 1.97.1 image, both able to bind 80 and 5004 | `cargo test --features cluster-integration-tests --test live_tv_two_node` | **executed, and all three cases failed on both. The conclusion recorded here — that this was the environment — was wrong; see the two rows below.** On media1 node B got as far as "serving authority recovered from a fresh quorum watermark" and the cases still failed after 298 s. The control failed **identically on both hosts**: the repo's own `cluster_activity` panics at `cluster_activity.rs:303` with `daemon readiness timed out`, 1 of 2 cases passing, while CI's `cluster_daemon` job is green on its self-hosted runner. Two independent hosts and a shared pre-existing baseline failure is as far as this can be taken without that runner.** The container binds both ports and sees itself on a private address the runtime accepts, so the fixture device is viable; the daemons then flood `hiqlite ... no such table: watched_outbox` and the cases fail after 300 s. **The control settles it**: the repo's own `cluster_activity` in the same container fails with `daemon readiness timed out` (1 of 2 cases passing), while CI's `cluster_daemon` job on its self-hosted runner is green. So this environment cannot host multi-daemon cluster tests, and that is a fact about the container rather than about the new cases. They still need a host like CI's runner, and are still unproven |
| 2026-09-05 | `main` at the merge base `3d847b58`, and the merged effort tree, on the **same** host in the **same** image | `cargo test -p plurxd --features cluster-integration-tests --test cluster_activity` | **the control that should have been run first.** `main` passes in 41.9 s; the effort tree times out at 90 s. So it was never the environment. Two independent hosts agreeing says nothing about whether the tree is at fault, and the four-minute experiment that does say so was available the whole time |
| 2026-09-05 | the joining node's own log, and `/readyz` on it | `wait_ready` now reports the last `/readyz` answer on timeout | **defect 1, in the product.** Node B answered `503 node_removal_fenced` for the full 90 s because its join could never be redeemed: `UPDATE cluster_join_tokens SET state = 'redeemed' … : no such table: main.settings`. `cluster_live_tv_join_reservation_guard` sits on a **cluster** table, which a joining node has from the first replicated entry, and read `settings`, which is application schema it does not have yet — and SQLite resolves a trigger's body when it PREPARES the guarded statement, so the guard did not stay quiet, it made the statement unpreparable. **No node could join a cluster running this build, whether or not Live TV was enabled.** Fixed by moving the enablement fact into a cluster-schema marker mirrored from `settings`, plus a census asserting that no trigger on a cluster table reads anything else |
| 2026-09-05 | the same three cases, run together and run one at a time | `cargo test --test live_tv_two_node -- --test-threads=1` | **defect 2, in the test file.** Each case passed alone; every case after the first failed together, on `owner_network: HDHomeRun device request failed`. `#[tokio::test]` drops its runtime when the case returns, and the fixture device had been spawned onto whichever runtime got there first, so it stopped listening after case 1 while the `OnceLock` kept handing out its address. The fixture now owns a thread and a runtime of its own |
| 2026-09-05 | all three cases, both fixes in | `cargo test -p plurxd --features cluster-integration-tests --test live_tv_two_node -- --test-threads=1` | **3 passed, 0 failed, 370.49 s.** The relay case, the drain-on-disable case, and the owner move each assert against two real daemons and a fixture device on ports 80 and 5004 |
...
| DRM unsupported | plurx has no licensed DRM path | a lawful DRM path exists |
| Captions carried, not advertised | the 608/708 fixture audit below shows A/53 captions survive every graph the default engine runs, but no rendition names them until the service ids real broadcasts use and each client's behaviour are recorded — a phantom or garbled track is worse than none | [LIVE-TV-SHARED-TRANSPORT.md](LIVE-TV-SHARED-TRANSPORT.md) M4 has the broadcast capture and the device pass |
...
## Caption audit per graph (L-03 M3)

What survives the live graphs, measured from bytes rather than assumed.
The fixture is generated by `live_tv/caption_audit_tests.rs`: 10 s of 1080i
MPEG-2 with B-frames and AC-3, four CC1 pop-on captions and the same four
lines as CEA-708 service 1, `cc_data` injected in display order. Each run
plans the route with the production planner from the production probe,
builds the production live argv (`live_ffmpeg_command_for_input`, tuner
input), feeds the fixture on stdin, joins the published HLS segments and
decodes CC1 and service 1 back out of FFmpeg's exported `cc_data`.
*Preserved* means every cue, in order, with its exact text, within 0.1 s of
its ground-truth time; *partial* means something decoded but not that;
*dropped* means nothing decoded. Every encoder row covers `send_field` and
`send_frame` deinterlacing and both MPEG-TS and fMP4 packaging (four runs).
Reproduce with `scripts/live-tv-caption-audit` (`--image` runs the fleet's
...
safe there, so nothing may be advertised from a node running it.
`send_field` copies each frame's `cc_data` onto both output fields, so CC1
text doubles ("PLPLURURX X") and every 708 packet repeats (*partial*);
`send_frame` is preserved. `h264_vaapi` in 5.1 has no `a53_cc` SEI flag at
all (*dropped*, and `-sei +a53_cc` would be a hard error there, not a fix).
The copy route is preserved. `h264_qsv` did not start on this build
(`Device creation failed` on `-init_hw_device qsv=hw` — a setup failure,
not a caption result). This is why advertising (M4) has to be keyed on the
FFmpeg build a node actually runs, not on the encoder family.
...
2. **~~Run the two-node cases~~ — done, and they found a real defect.** `make live-tv-two-node-check`: all three pass in 370 s in a container that binds 80 and 5004. They had been failing on two separate causes, and the earlier entry in this document blamed neither correctly. Read that entry before trusting a "the environment cannot host this" conclusion again: two hosts agreeing is not evidence about the environment, and the control that settles it — `main` at the merge base on the same box — takes four minutes.
