...
**Status:** M0-M3, M4 (partial), M6, M7 and one found fix merged from [PR #502](http://192.168.4.7:3000/noirr/plurx/pulls/502) as `4bba363bc`; M5 in draft [PR #541](http://192.168.4.7:3000/noirr/plurx/pulls/541); M4's keyset pagination and `next_up` CTE not built by decision; post-merge fleet evidence owed (see the execution log) · **Executes:** S6, S7, S11, F-sc-6, F-sc-7,
...
| 2026-09-24 | claude-opus-5-5 | https://claude.ai/code/session_01AZemhL7Y1nXGWxUGRC2tkK | M1 | [#502](http://192.168.4.7:3000/noirr/plurx/pulls/502) | `51bb100a8`: 16 methods move to the read pool (the commit message says 17; the method list here and the diff have 16) — `list_top_items_in_genre` (on the new `with_read_txn`, `BEGIN DEFERRED` held for the closure, so page and count come from one WAL snapshot), `recently_added`, `search_items`, `get_item_children`, `episodes_for_show`, `files_for_item`, `child_counts`, `item_max_heights`, `item_media_facts` (media.rs); `watch_map`, `watch_rollup`, `watch_rollups`, `continue_watching`, `next_up` (watch.rs); `get_library`, `list_libraries` (library.rs). Section 3.1 checklist, all 16: **(1) no DML** — each closure is `SELECT`/`WITH … SELECT` only, and the helpers it reaches (`item_from_row`, `file_from_row`, `watch_from_row`, `library_from_row`, `sql_source::next_up`, `sql_source::recently_added`) build values or SQL text and execute nothing; **(2) snapshot** — every closure is one statement except `list_top_items_in_genre` (count + page, `with_read_txn`) and, after M4, `recently_added` (one statement per widening pass; each pass is exact for the state it read, so it declares no shared snapshot); **(3) not pre-mutation** — none decides a write in the same closure; their callers are the browse, Plex and Home handlers, which render; the scanner's lookups that decide inserts (`find_*`, `item_by_external_id`, `get_file_by_path`) deliberately stay on the writer; **(4) not dependent on the writer's uncommitted state** — all are trait methods, never called from inside a writer closure. Premise correction: the plan's example of a pre-mutation read, `get_item` before `set_watched_tree`, was already on `with_read` before this plan (`media.rs:701`), and line numbers in section 2.4 have drifted. Tests: `home_and_library_page_reads_do_not_queue_behind_a_held_writer_transaction` (17 reads: the 16 moved here plus `home_preview_pages`, which was already on `with_read`; every one returns within a 2 s bound while the writer holds `BEGIN IMMEDIATE` with an uncommitted insert, and none sees it; the writer is released only after all return, so a regression times out rather than passing slowly — a stronger form of the plan's "< 100 ms under a 2 s hold") and `read_transactions_see_one_snapshot_across_statements` (a commit between two statements is invisible under `with_read_txn`, visible under `with_read` as the control). Reverting `list_libraries` to `with_conn` and `with_read_txn` to a bare `f(conn)` fails each. |
...
| 2026-09-25 | claude-opus-5-5 | https://claude.ai/code/session_01AZemhL7Y1nXGWxUGRC2tkK | Fix (found in M4) | [#502](http://192.168.4.7:3000/noirr/plurx/pulls/502) | `ea62fa80d`: the replicated Authority `recently_added` (the consistent path the bounded catalogue reader falls back to, and every caller of the trait method on a cluster) now leaves Recordings libraries out of the catalogue-wide rail like the standalone store and the replicated local read; the `exclude_recordings` flag is gone from `sql_source::recently_added`. `catalogue_recently_added_leaves_out_recordings_on_every_backend` (store contract: in-memory and file SQLite, three Hiqlite voters) asserts the catalogue rail is exactly the movie and the Recordings library's own rail exactly the recording; restoring the old Authority statement fails it on `hiqlite-3-voter` (left `[2, 1]`, right `[1]`). Coverage: the pull request's `Regression-Test:` line for that test. The Hiqlite "after" timings in the evidence doc predate this predicate. |
| 2026-09-25 | claude-opus-5-5 | https://claude.ai/code/session_01AZemhL7Y1nXGWxUGRC2tkK | Gate follow-ups | [#502](http://192.168.4.7:3000/noirr/plurx/pulls/502) | The first unfiltered `cargo test -p plurx-core -p plurxd` on the branch (earlier runs were filtered to `store::sqlite`) failed four `plurx-core` unit tests, all branch defects, all resolved here: `22d70ba65` widening kept the `i64::MAX` "no bound" offset at `MAX - 1` (no rail changed; `recently_added_windows_start_at_eight_rows_a_card_and_double` pins it, coverage by the pull request's `Regression-Test:` line); `d7fd4dd7b` registers `housekeeping.rs` in the placeholder census and the transaction-site registry, registers `with_read_txn`'s `BEGIN DEFERRED` as a new `ReadSnapshot` site (98 → 99; `with_read_txn` is now private to the `sqlite` module so the registry can bound the method before it), and re-measures the unchecked-arity census 93 → 95 site by site against `origin/main` (the tracing binds `list_top_items_in_genre`'s count, `item_by_external_id` and `authenticate_token`'s read to a name before running them; `recently_added` left the directory for `sql_source`). Also `c326c7c5b` (an unused import clippy refused) `c0d50794e` (the known-red ignore census counts the M0 capture test, 17 → 18), and `9dc7a194b` (the decoder-recovery status contract reads the 99). |
| 2026-09-25 | claude-opus-5-5 | https://claude.ai/code/session_01AZemhL7Y1nXGWxUGRC2tkK | Review of #502 | [#502](http://192.168.4.7:3000/noirr/plurx/pulls/502) | Review comment 4635, two findings. **(1) Deviation from section 3.9:** `5272038fa` drops `PRAGMA quick_check(1)` from poisoned-connection recovery. It ran with the slot's mutex held and reads every page of a healthy file (the `1` caps only the errors reported), so a closure panicking on every call put a whole-file read in front of each later caller on its slot; a Rust panic cannot damage the file, which is checked at boot within its 30 s budget. Recovery now rolls back an open transaction, requires autocommit, runs `SELECT 1`, and checks the writer's foreign keys. `recovering_a_poisoned_connection_does_not_read_the_database` counts the pages a recovery reads with SQLite's per-connection cache-miss counter on a cold 452-page file, on both pools, requires at most 4 and the same connection kept, and checks the counter with a `quick_check` control (>400); with `quick_check` restored in `validate` it fails ("Writer: recovery read 451 of 452 pages"); coverage by the pull request's `Regression-Test:` line. **(2)** M1 moved 16 reads, not 17 (nine in `media.rs`, five in `watch.rs`, two in `library.rs`); the M1 row, the work-board row and the PR body now say 16, and the M1 test's seventeenth read, `home_preview_pages`, was already on the read pool. The M1 commit message keeps its 17. Merged `origin/main` at `448e803da` as `dcba5e28f` (clean; `cargo test -p plurx-core -p plurxd` exit 0 before any fix). |
| 2026-09-25 | claude-opus-5-5 | https://claude.ai/code/session_01AZemhL7Y1nXGWxUGRC2tkK | Promotion | [#502](http://192.168.4.7:3000/noirr/plurx/pulls/502) | On merging `origin/main` (`37baf6e0b`, P-03 phase B in force past `448e803da`) the branch's four `validation/regressions.d` rows (`213820c2`, `22d70ba6`, `5272038f`, `ea62fa80`) were refused by the freeze, since all four commits are past the boundary. They are removed; the pull request body's four `Regression-Test:` lines name the same tests and are carried into the landing commit's message. |
| 2026-09-25 | claude-opus-5-5 | https://claude.ai/code/session_01AZemhL7Y1nXGWxUGRC2tkK | M5 | [#541](http://192.168.4.7:3000/noirr/plurx/pulls/541) | `f782fc24f`: SQLite schema v70 and replicated schema v48 add three indexes on `items`, one `sql_source::ITEM_READ_INDEXES` constant for both (`CREATE INDEX IF NOT EXISTS` throughout; fresh clusters get them from the catalogue schema, an existing one through one Raft transaction with the marker). **Adopted:** `idx_items_tmdb` and `idx_items_imdb` (partial, the IMDb one `COLLATE NOCASE`) for `item_by_external_id`, 3.256 → 0.022 ms warm on SQLite and 4.037 → 0.026 ms on the Hiqlite schema; and `idx_items_top_level_title`, `(library_id, sort_title)` partial on `TOP_LEVEL_ITEM_PREDICATE`, which takes the Title page's temp B-tree away (8.8 → 0.27 ms at offset 10,000) and makes every sort's unfiltered count read only top-level entries (1.4-1.8 → 0.41-0.52 ms; the shows library's count 5.9 → 0.023 ms), `library_page_title` 10.5 → 0.78 ms per call, `shows_page_title` 12.3 → 0.087 ms, `home_preview_pages` 25 → 18 ms; no other statement's plan changed, on either schema, with or without statistics. **Premise corrections:** `item_by_external_id` is not a scanner lookup — its one production caller is the coming-soon rail's artwork pass (`crates/plurxd/src/http/comingsoon.rs:415`), one lookup per calendar entry per request, serially; the §3.4 candidate `items(library_id, kind, sort_title, id)` cannot serve the page because `kind` is an `IN`/`OR` there, and a plain `(library_id, sort_title)` was measured and rejected (the planner served every count from it and seeked the table per entry for `kind`); the bundled planner (3.53.2) would not drive the `OR` from the new external-id indexes (still `SCAN items USING INDEX idx_items_library_kind`), so the statement moved to `sql_source` as `kind = ? AND id IN (tmdb arm UNION ALL imdb arm)` with the old `ORDER BY` verbatim — the same rows in the same order, pinned against the old statement over 200 seeded catalogues. **Rejected with measurements:** `(library_id, sort_title, kind, parent_id)`, `(library_id, sort_title, id, kind, parent_id)`, and a partial `(library_id, added_at DESC, id DESC)` (Added page 21.7 → 0.33 ms, but the planner then reads Year and Resolution through it: 23.7 → 27.5 and 43.5 → 61.4 ms). **Migration cost** (Hiqlite fixture, state-machine pragmas): 63 ms cold / 59 ms warm median for all three; 688 KiB of index. Openraft 0.9.25 applies entries in its own state-machine task, so heartbeats do not wait on the build. Tests: `item_by_external_id_searches_one_partial_index_per_arm`, `item_by_external_id_matches_the_or_statement`, `library_pages_read_top_level_items_from_the_title_index` (`sqlite/media.rs`), `item_by_external_id_renders_equivalent_valid_dialects_from_one_parameter_order`, `top_level_title_index_is_the_top_level_predicate` (`sql_source.rs`), `sqlite_v70_migration_adds_the_read_indexes_and_keeps_the_catalogue`, `replicated_v47_store_migrates_the_read_indexes_on_daemon_open` (`store_contract.rs`, three voters). Removing the three `CREATE INDEX` statements fails the first, third, fifth and the SQLite migration test (e.g. `SCAN items` in both arms; `idx_items_library_kind` for the counts; 0 indexes, not 3); restoring the `OR` statement with the indexes kept fails the first (`SCAN items USING INDEX idx_items_library_kind`). The unchecked SQLite arity census is 97 → 96 (`item_by_external_id`'s site left `sqlite/media.rs` with the statement). Evidence: `benchmarks/evidence/query-plans-m5-f782fc24.md`; procedure and cost in OPERATIONS.md ("Catalogue read indexes: SQLite v70, replicated schema v48"). |
| 2026-09-25 | claude-opus-5-5 | https://claude.ai/code/session_01AZemhL7Y1nXGWxUGRC2tkK | M4 remainder (decision) | [#541](http://192.168.4.7:3000/noirr/plurx/pulls/541) | **Not built, by decision.** Keyset pagination: no endpoint takes a cursor today, so a keyset page is an API change every client (web, Android, Apple) would have to adopt before it did anything, and M5's partial title index already removes the sort the keyset was meant to remove for the default Title grid (0.27 ms at offset 10,000); the `OFFSET` walk that remains is over index entries, not a sort. The `next_up` CTE: M0 refuted its premise — SQLite evaluates three of the four subqueries once as list subqueries with Bloom filters, not per episode row; only the `MAX` is per show (8.1-9.3 ms warm on the fixture) — so the rewrite and its 200-seed equivalence test would buy little against their risk. The pagination order-equivalence test's tiebreak half already landed (`37b974059`). |
| 2026-09-25 | claude-opus-5-5 | https://claude.ai/code/session_01AZemhL7Y1nXGWxUGRC2tkK | Finding (statistics on Hiqlite) | [#541](http://192.168.4.7:3000/noirr/plurx/pulls/541) | The M0 plans assumed no statistics on either backend, and M6 rejected `ANALYZE` because `watch_rollups` became 3.5× slower. The vendored Hiqlite state machine runs `PRAGMA optimize=0x10002` on every connection it opens and `PRAGMA optimize` after each snapshot, migration and shutdown (`vendor/hiqlite/src/store/state_machine/sqlite/state_machine.rs:593`, `writer.rs:496-706`), so replicated voters do have `sqlite_stat1`, collected per voter outside the Raft log; a read-only copy of the nuc3 voter's current snapshot (schema 47) has rows for every `items` and `watch_state` index. On the Hiqlite fixture with those statistics `watch_rollups` is 7.9 ms against 2.6 ms without — the M6 regression, already live on the replicated backend. Open question 2 therefore does arise, in the other direction (whether to keep the vendor's statistics, pin plans, or rewrite `watch_rollups`); not changed here. M5's plans were taken both ways and choose the new indexes either way. |
| 2026-09-25 | claude-opus-5-5 | https://claude.ai/code/session_01AZemhL7Y1nXGWxUGRC2tkK | needs: fleet evidence (M5) | [#541](http://192.168.4.7:3000/noirr/plurx/pulls/541) | Post-merge, for the GPT session with fleet access: "K-05 M5 (PR #541) adds three `items` indexes as SQLite v70 / replicated schema v48. (1) Before the maintenance window, on every voter: read `<PLURX_DATA>/hiqlite/state_machine/snapshots/current` for the snapshot file name, copy that file to /tmp, and with python3's sqlite3 on the copy record `SELECT schema_version FROM cluster_meta`, `SELECT count(*) FROM items`, and `SELECT idx, stat FROM sqlite_stat1 WHERE tbl='items'`; then time, on the same copy, `BEGIN; CREATE INDEX IF NOT EXISTS idx_items_tmdb ON items(tmdb_id) WHERE tmdb_id IS NOT NULL; CREATE INDEX IF NOT EXISTS idx_items_imdb ON items(imdb_id COLLATE NOCASE) WHERE imdb_id IS NOT NULL; CREATE INDEX IF NOT EXISTS idx_items_top_level_title ON items(library_id, sort_title) WHERE (kind IN ('movie','show','book','audiobook') OR (kind IN ('folder','video','photo') AND parent_id IS NULL)); COMMIT;` with `PRAGMA journal_mode=WAL; PRAGMA synchronous=OFF` first (five runs, report the median), and delete the copy. Never open the live `state_machine/db/plurx.db`. (2) Deploy with the Ansible playbook following docs/OPERATIONS.md 'Catalogue read indexes: SQLite v70, replicated schema v48': stop application traffic, update every voter, then learners, as one operation; record each voter's restart-to-`/readyz` time beside the previous deploy's. (3) After the next snapshot on each voter, repeat the copy and confirm `schema_version` 48, the three index names in `sqlite_master`, and `sqlite_stat1` rows for them; run `EXPLAIN QUERY PLAN` for the statement in `sql_source::item_by_external_id` (bind 'movie', a real tmdb id, NULL) and for `SELECT COUNT(*) FROM items WHERE library_id = 1 AND (kind IN ('movie','show','book','audiobook') OR (kind IN ('folder','video','photo') AND parent_id IS NULL)) AND (NULL IS NULL)` and report whether they name `idx_items_tmdb`/`idx_items_imdb` and `idx_items_top_level_title`. (4) With the 'Measuring page-route latency' procedure in docs/OPERATIONS.md, take the Home (coming-soon rail) and a movie library's default Title grid before and after. Append the results to this execution log and the K-05 board row in one evidence-only docs PR." |
