...
enum Caps {
    ...
    static func controlCapabilities(
        ...
        return DynamicCapabilities(
            ...
            // Measured, not assumed. M5.5 ran on hardware on 2026-09-01: both
            // required devices — iPhone 17 Pro Max and Apple TV 4K (3rd
            // generation) — completed 20/20 clean commits in *both* the
            // same-codec and the codec/HDR case, with zero predecessor and
            // zero post-commit stalls across eighty acknowledgements, each
            // holding two live `AVPlayer` pipelines with both outputs
            // producing frames.
            //
            // The first instrument set was rejected and the corrective pass
            // repaired it: 132-second fixtures so the runway varies and is
            // never full at acknowledgement, a copied pixel buffer whose
            // presentation timestamp is required to fall at or beyond the
            // commit boundary, and per-trial wire counts off the shaping
            // proxy. First-frame latency then separated the two cases —
