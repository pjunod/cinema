...
final class PlaybackControlSessionTests: XCTestCase {
    ...
    private func registerCleanup(
        ...
        addTeardownBlock { @MainActor in
            session.end()
            // `end()` deliberately detaches its bounded final exchange.
            // Invalidating the fixture session before that bound expires is
            // process-fatal in CFNetwork rather than a catchable send error.
            // Retain the delegate-free ephemeral session through the exact
            // production bound, then let ordinary ownership release it.
            try await Task.sleep(
                nanoseconds: UInt64(PlaybackControlReporter.finalizationDeadlineMs + 100)
                    * 1_000_000
            )
            withExtendedLifetime(urlSession) {}
        }
    }
    ...
    func testAStartedSessionExchangesWhatThePlayerReported() async throws {
        ...
        registerCleanup(session, urlSession)
