 ...
 */
/**
 * The prewarm counter shipped before any callsite could produce a hit, so
 * `plurx_playback_marker_prewarm_hit_ratio` read zero permanently and an
 * absent feature looked like a broken one. These assertions keep the hit
 * branch reachable and keep the two "buffered but still not prewarmed" cases
 * honest.
 */
class MarkerPrewarmTest {

    @Test
    fun aBufferedNativeDestinationIsAHit() {
        assertEquals(
            "hit",
            markerPrewarmResult(
                nativeSeek = true,
                destinationMs = 90_000,
                positionMs = 60_000,
    ...
    @Test
    fun aDestinationBeyondTheBufferIsAMiss() {
        assertEquals(
            "miss",
            markerPrewarmResult(
                nativeSeek = true,
    ...
    /** A reopen re-requests the stream; the buffer says nothing about it. */
    @Test
    fun aReopenIsAMissHoweverFullTheBufferIs() {
        assertEquals(
            "miss",
            markerPrewarmResult(
                nativeSeek = false,
                destinationMs = 90_000,
                positionMs = 60_000,
                bufferedPositionMs = 600_000,
            ),
        )
    }

    /** ExoPlayer's buffer is a forward window, and a skip goes forward. */
    @Test
    fun aDestinationBehindThePlayheadIsNeverAPrewarm() {
        assertEquals(
            "miss",
            markerPrewarmResult(
                nativeSeek = true,
                destinationMs = 30_000,
                positionMs = 60_000,
                bufferedPositionMs = 120_000,
            ),
        )
    ...
    /** The boundary is inclusive: the last buffered millisecond is loaded. */
    @Test
    fun theBufferEdgeItselfIsAHit() {
        assertEquals(
            "hit",
            markerPrewarmResult(
                nativeSeek = true,
