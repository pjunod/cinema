...
> and TV. Build 120 excludes the credential DataStore from Auto Backup and from
> device-to-device transfer, and gives the release variant a signing config that
> fails rather than falling back to the debug key, so the APK the server serves
> is release-signed and not `debuggable`. Build 110 executes a server-required copy-HLS remux on initial play,
