...
impl RpuDataMapping {
    pub(crate) fn parse(
        ...
        for cmp in 0..NUM_COMPONENTS {
            ...
            // PLURX-PATCH 1: `num_pivots_minus2` is an unbounded ue(v) and
            // sized a `vec![0; num_pivots]` of u16 directly, so a corrupted
            // RPU could request gigabytes and abort the process. Each pivot
            // is read as `bl_bit_depth` bits, so a count that does not fit in
            // the bits left is refused before the allocation.
            let available_bits = reader.available()?;
            let num_pivots_u64 = curve
                .num_pivots_minus2
                .checked_add(2)
                .filter(|pivots| *pivots <= available_bits / u64::from(bl_bit_depth))
                .ok_or_else(|| {
                    anyhow::anyhow!(
                        "num_pivots_minus2 {} cannot fit in the {} bits left in the RPU",
                        curve.num_pivots_minus2,
                        available_bits
                    )
                })?;
            let num_pivots = num_pivots_u64 as usize;
...
impl DoviPolynomialCurve {
    ...
    fn parse(&mut self, reader: &mut BsIoSliceReader, header: &RpuDataHeader) -> Result<()> {
        ...
        if poly_order_minus1 == 0 && linear_interp_flag {
            ...
            // PLURX-PATCH 2: upstream stops here with `unimplemented!()`, a
            // panic that any RPU can trigger with two bits. A parser reached
            // from untrusted media has to refuse, not unwind.
            bail!("parse: polynomial linear interpolation is not supported");
