...
impl ExtMetadataBlockLevel8 {
    pub(crate) fn parse(reader: &mut BsIoSliceReader, length: u64) -> Result<ExtMetadataBlock> {
        // PLURX-PATCH 3: `length` comes from the bitstream, and every other
        // value fell through to `unreachable!()` in `required_bits` once the
        // block was validated. Refuse the unknown length here instead.
        ensure!(
            matches!(length, 10 | 12 | 13 | 19 | 25),
            "level 8 block length {} is not one this parser knows",
            length
        );
        let mut block = Self {
            length,
