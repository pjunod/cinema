...
/// The smallest encoding of one extension block: `ext_block_length` as ue(0)
...
impl DmData {
    pub(crate) fn parse<T: WithExtMetadataBlocks + Default>(
        ...
        // PLURX-PATCH 1: `num_ext_blocks` is an unbounded ue(v) read from the
        // bitstream and was used as a `Vec::with_capacity` argument, so a
        // corrupted or hostile RPU could ask for tens of gigabytes and abort
        // the process. Every block costs at least ue(0) for its length plus
        // an 8-bit level, so a count larger than the bits left divided by 9
        // cannot be parsed and is refused before anything is allocated.
        let available_bits = reader.available()?;
        ensure!(
            num_ext_blocks <= available_bits / MIN_EXT_BLOCK_BITS,
            "num_ext_blocks {} cannot fit in the {} bits left in the RPU",
            num_ext_blocks,
            available_bits
        );
